...
class MutationObserver {
  observer(
    _changes: Array<
      KeyValueTuple<FILTER_CRITERIA_UNION | SORT_CRITERIA_UNION, string>
    >,
  ): void {
    // No-op.
  }
...
describe("FiltersSortManager", () => {
  ...
  beforeEach(() => {
    urlStateManagerStub = jasmine.createSpyObj<URLStateManager>(
      "urlStateManagerStub",
      [
        "changeBaseUrl",
        "locationToURL",
  ...
  describe("Methods::", () => {
    ...
    describe("|clear|", () => {
      it("should verify will invoke correct methods with correct parameters", () => {
        // Given
        const clearFiltersSpy = spyOn(manager, "clearFilters").and.callFake(
          CallFake,
        );
    ...
    describe("|bulkUpdate|", () => {
      ...
      it(`should verify will update values from Array of key-value (criteria-value) tuples, filter and sort`, fakeAsync(() => {
        ...
        manager.bulkUpdate([
          ...
          [SORT_CRITERIA_S3, null, "sort"],
          [FILTER_CRITERIA_F2, null, "filter"],
          ["someSort", "-1", "sort"] as unknown as KeyValueTuple<
            SORT_CRITERIA_UNION,
            string
          >,
          [FILTER_CRITERIA_F4, filterValue4, "filter"],
          ["someFilter", "testValue", "filter"] as unknown as KeyValueTuple<
            FILTER_CRITERIA_UNION,
            string
          >,
          [SORT_CRITERIA_S2, sortValue2, "sort"],
        ...
        expect(observerSpy1).toHaveBeenCalledWith([
          ...
          [
            SORT_CRITERIA_S2,
            parseInt(sortValue2, 10) as unknown as string,
            "sort",
          ],
          [SORT_CRITERIA_S3, null, "sort"],
      ...
      it(`should verify will update values from object of key-value (criteria-value) properties, filter and sort are missing and clear previous values`, fakeAsync(() => {
        ...
        // When
        manager.bulkUpdate(
          {} as Record<typeof FILTER_KEY | typeof SORT_KEY, string>,
          true,
        );
