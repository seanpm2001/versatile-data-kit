...
> = [key: K, value: V, type?: typeof FILTER_KEY | typeof SORT_KEY];
/**
...
> {
   ...
   * @private
   */
  private _updateStrategy: "navigateToUrl" | "locationToURL" | "replaceToURL" =
    "locationToURL";
  /**
   ...
   * @private
   */
  private readonly _mutationObservers: Set<
    FilterSortMutationObserver<FC, FV, SC, SV>
  > = new Set<FilterSortMutationObserver<FC, FV, SC, SV>>();
  ...
  hasFilter(criteria: FC): boolean {
    return (
      this.filterCriteria.hasOwnProperty(criteria) &&
      CollectionsUtil.isDefined(this.filterCriteria[criteria])
    );
  }
  ...
  hasAnyFilter(): boolean {
    return (
      CollectionsUtil.objectPairs(this.filterCriteria).filter(([_key, value]) =>
        CollectionsUtil.isDefined(value),
      ).length > 0
    );
  }
  ...
  clearFilters(updateBrowserUrl = true, notifyMutationObservers = true): void {
    ...
    const mutatedFilterPairs: KeyValueTuple<FC, FV>[] = filterPairs.map(
      ([key]) => [key, null, "filter"] as KeyValueTuple<FC, FV>,
    );
  ...
  hasSort(criteria: SC): boolean {
    return (
      this.sortCriteria.hasOwnProperty(criteria) &&
      CollectionsUtil.isDefined(this.sortCriteria[criteria])
    );
  }
  ...
  hasAnySort(): boolean {
    return (
      CollectionsUtil.objectPairs(this.sortCriteria).filter(([_key, value]) =>
        CollectionsUtil.isDefined(value),
      ).length > 0
    );
  }
   ...
   */
  bulkUpdate(
    filterSortPairs: KeyValueTuple<FC | SC, FV | SV>[],
   ...
   */
  bulkUpdate(
    filterValues: Record<typeof FILTER_KEY | typeof SORT_KEY, string>,
    clearPreviousValues?: boolean,
  ): void;
  /**
   * @inheritDoc
  ...
  bulkUpdate(
    updates:
      | Record<typeof FILTER_KEY | typeof SORT_KEY, string>
      | KeyValueTuple<FC | SC, FV | SV>[],
    ...
    if (
      (updates as object).hasOwnProperty(FILTER_KEY) &&
      CollectionsUtil.isStringWithContent(updates[FILTER_KEY])
    ) {
      const deserializedFilterCriteria = this._deserializeFilters(
        updates[FILTER_KEY],
      );
      const normalizedFilterCriteria: KeyValueTuple<FC, FV>[] =
        CollectionsUtil.objectPairs(deserializedFilterCriteria).map(
          (filterPairs) =>
            [
              filterPairs[0],
              filterPairs[1],
              "filter",
            ] as unknown as KeyValueTuple<FC, FV>,
        );

      const mutatedFilterPairs: KeyValueTuple<FC, FV>[] =
        this._persistBulkFilters(normalizedFilterCriteria, clearPreviousValues);
      this._serializeFilters();
    ...
    if (
      (updates as object).hasOwnProperty(SORT_KEY) &&
      CollectionsUtil.isStringWithContent(updates[SORT_KEY])
    ) {
      const deserializedSortCriteria = this._deserializeSort(updates[SORT_KEY]);
      const normalizedSortCriteria: KeyValueTuple<SC, SV>[] =
        CollectionsUtil.objectPairs(deserializedSortCriteria).map(
          (sortPairs) =>
            [sortPairs[0], sortPairs[1], "sort"] as unknown as KeyValueTuple<
              SC,
              SV
            >,
        );
   ...
   */
  updateBrowserUrl(
    updateStrategy?: "navigateToUrl" | "locationToURL" | "replaceToURL",
    skipDebouncing = false,
  ): void {
    const strategy = CollectionsUtil.isStringWithContent(updateStrategy)
      ? updateStrategy
  ...
  changeUpdateStrategy(
    strategy: "navigateToUrl" | "locationToURL" | "replaceToURL",
  ): void {
  ...
  changeDebouncingTime(debouncingTime: number): void {
    if (
      !CollectionsUtil.isNumber(debouncingTime) ||
      CollectionsUtil.isNaN(debouncingTime)
    ) {
      return;
    }
  ...
  registerMutationObserver(
    callback: FilterSortMutationObserver<FC, FV, SC, SV>,
  ): void {
   ...
   * @private
   */
  private _persistBulkFilters(
    filterPairs: KeyValueTuple<FC, FV>[],
    clearPreviousValues: boolean,
  ): KeyValueTuple<FC, FV>[] {
    const mutatedFilterPairs: KeyValueTuple<FC, FV>[] = [];

    for (const knownCriteria of this.knownFilterCriteria) {
      const foundFilterPairs = filterPairs.filter(
        ([criteria, _value, type]) =>
          criteria === knownCriteria && type === "filter",
      );
   ...
   * @private
   */
  private _persistBulkSort(
    sortPairs: KeyValueTuple<SC, SV>[],
    clearPreviousValues: boolean,
  ): KeyValueTuple<SC, SV>[] {
    const mutatedSortPairs: KeyValueTuple<SC, SV>[] = [];

    for (const knownCriteria of this.knownSortCriteria) {
      const foundSortPairs = sortPairs.filter(
        ([criteria, _direction, type]) =>
          criteria === knownCriteria && type === "sort",
      );
  ...
  private _serializeFilters(): void {
    const filterPairs = CollectionsUtil.objectPairs(this.filterCriteria);
    const normalizedFilterPairs: string =
      filterPairs.length > 0 ? JSON.stringify(this.filterCriteria) : null;
  ...
  private _serializeSort(): void {
    ...
    const normalizedSortPairs: string =
      sortPairs.length > 0 ? JSON.stringify(this.sortCriteria) : null;
  ...
  private _updateUrlStateManager(
    ...updatePairs: KeyValueTuple<typeof FILTER_KEY | typeof SORT_KEY, string>[]
  ): void {
  ...
  private _notifyMutationObservers(
    changes: KeyValueTuple<FC | SC, FV | SV>[],
  ): void {
    if (changes.length === 0) {
      return;
    }
