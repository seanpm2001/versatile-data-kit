    ...
    OnTaurusModelError
{
      ...
      this.routerService
        ...
        .subscribe((routerState) => {
          if (initializationFinished) {
            ...
            if (
              (!previousState ||
                previousState.absoluteRoutePath ===
                  routerState.state.absoluteRoutePath) &&
  ...
  private _shouldRestoreUIState(routerState: RouterState): boolean {
    ...
    return routerState
      .getPrevious()
      .state.absoluteConfigPath.includes(restoreUiWhen.previousConfigPathLike);
  }
  ...
  private _extractQueryParams(routeState: RouteState): void {
    ...
    if (!this.initializingComponent) {
      ...
      this.clrGridUIState.filter.teamName =
        routeState.getQueryParam("teamName");
      this.clrGridUIState.filter.description =
        routeState.getQueryParam("description");
      this.clrGridUIState.filter.deploymentStatus =
        this._decodeFilterFromQueryParam(
          "deploymentStatus",
      ...
      this._checkMutatedFilterAndUpdate(
        routeState,
        "deploymentLastExecutionStatus",
        true,
      );
  ...
  private _updateUrlStateManager(routeState?: RouteState): void {
    ...
    this.urlStateManager.setQueryParam(
      ...
      this.filtersQueryParamPositionBase + 1,
    );
    this.urlStateManager.setQueryParam(
      "teamName",
      this.clrGridUIState.filter.teamName,
      this.filtersQueryParamPositionBase + 2,
    );
    this.urlStateManager.setQueryParam(
      "description",
      this.clrGridUIState.filter.description,
      this.filtersQueryParamPositionBase + 3,
    );
    this.urlStateManager.setQueryParam(
      "deploymentStatus",
      this._encodeFilterForQueryParam(
        "deploymentStatus",
        this.clrGridUIState.filter.deploymentStatus,
      ),
    ...
    this.urlStateManager.setQueryParam(
      this.searchParam,
      this.clrGridUIState.search,
      this.searchParamPosition,
    );
  }

  private _areQueryParamsPristine(routeState: RouteState): boolean {
    if (
      this.clrGridUIState.search !== routeState.getQueryParam(this.searchParam)
    ) {
      return false;
    }
      ...
      if (
        CollectionsUtil.isDefined(
          routeState.getQueryParam(key) &&
            this.clrGridUIState.filter[key] !==
              this._decodeFilterFromQueryParam(
                key,
                routeState.getQueryParam(key),
              ),
        ...
        this.clrGridUIState.filter[key] = this._decodeFilterFromQueryParam(
          key,
          routeState.getQueryParam(key),
        );
      }
  ...
  private _doUrlUpdate(
    strategy: "updateLocation" | "updateRouter" | "replaceLocation" = this
      .urlUpdateStrategy,
  ): void {
  ...
  private _encodeFilterForQueryParam(
    propertyName: keyof GridFilters,
    value: string,
  ): string {
    switch (propertyName) {
  ...
  private _decodeFilterFromQueryParam(
    ...
    switch (propertyName) {
      ...
      case "deploymentLastExecutionStatus":
        ...
        const normalizedExecStatus: DataJobExecutionStatus =
          `${value}`.toUpperCase() as DataJobExecutionStatus;

        return this.executionStatuses.includes(normalizedExecStatus)
          ? normalizedExecStatus
          : undefined;
      default:
        return `${value}`.toLowerCase();
    }
  ...
  private _initializeQuickFilters(): void {
    ...
    const filters: QuickFilters = [
      ...
      },
      {
        ...
        icon: {
          title:
            "Disabled - This job is deployed but not executing by schedule",
