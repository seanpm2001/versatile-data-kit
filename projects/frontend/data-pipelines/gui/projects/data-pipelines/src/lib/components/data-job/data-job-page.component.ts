  ...
  implements OnInit, OnTaurusModelInit, OnTaurusModelError
{
              ...
              interval(1250) // Send polling request on every 1.25s until execution is accepted from backend
                .pipe(
                  ...
                  map((executions: DataJobExecutionsPage) =>
                    executions.content ? [...executions.content] : [],
                  ),
                  takeWhile((executions) => {
                    if (
                      CollectionsUtil.isArrayEmpty(executions) ||
                      executions.length <= this.jobExecutions.length
                    ) {
                      return true;
                    }
        ...
        .pipe(
          switchMap((id) =>
            interval(5000).pipe(
              switchMap(() =>
                this.dataJobsApiService
                  .getJobExecution(this.teamName, this.jobName, id)
                  .pipe(
                    map((execution) => {
                      return {
                        execution,
              ...
              tap((data) =>
                this._replaceRunningExecutionAndNotify(data.execution),
              ),
              takeWhile((data) => {
                ...
                const isRunning =
                  ...
                  DataJobUtil.isJobRunningPredicate(data.execution);
      ...
      this.dataJobsService
        ...
        .pipe(
          concatMap((executionId: string) =>
            this.dataJobsApiService
              .getJobExecution(this.teamName, this.jobName, executionId)
              .pipe(
        ...
        next: (job) => {
          if (CollectionsUtil.isDefined(job)) {
            ...
            this.isExecuteJobAllowed =
              ExtractJobStatusPipe.transform(this.jobDeployments) !==
  ...
  private _replaceRunningExecutionAndNotify(
    ...
    const convertedExecution =
      DataJobUtil.convertFromExecutionDetailsToExecutionState(executionDetails);
