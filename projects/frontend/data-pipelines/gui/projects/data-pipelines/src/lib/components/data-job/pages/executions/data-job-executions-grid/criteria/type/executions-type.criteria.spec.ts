...
describe("ExecutionsTypeCriteria", () => {
  ...
  beforeEach(() => {
    ...
    dataJobExecutions = [
      {
        id: "aJob",
        startTimeFormatted: datePipe.transform(
          aStartTime,
          DATA_PIPELINES_DATE_TIME_FORMAT,
        ),
        startTime: aStartTime.toISOString(),
        endTimeFormatted: datePipe.transform(
          aEndTime,
          DATA_PIPELINES_DATE_TIME_FORMAT,
        ),
  ...
  describe("Methods::", () => {
    describe("|meetCriteria|", () => {
      ...
      it("should verify will return Array with all Jobs when serialized status deserialization fails", () => {
        // Given
        spyOn(CollectionsUtil, "isStringWithContent").and.throwError(
          new Error("String validation fails"),
        );
