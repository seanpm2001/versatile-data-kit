...
export class ExecutionsTypeCriteria implements Criteria<GridDataJobExecution> {
  ...
  constructor(dataJobExecutionTypesSerialized: string) {
    this._dataJobExecutionTypes =
      ExecutionsTypeCriteria._deserializeExecutionTypes(
        dataJobExecutionTypesSerialized,
      );
  }
  ...
  private static _deserializeExecutionTypes(
    dataJobExecutionTypesSerialized: string,
  ): DataJobExecutionType[] {
    try {
      if (
        !CollectionsUtil.isStringWithContent(dataJobExecutionTypesSerialized)
      ) {
        return [];
      }

      return dataJobExecutionTypesSerialized
        .toUpperCase()
        .split(",") as DataJobExecutionType[];
    } catch (e) {
      console.error(
        `ExecutionsTypeCriteria: failed to deserialize Data Job Execution Types.`,
      );
