...
import {
  ExecutionsStatusCriteria,
  ExecutionsStringCriteria,
  ExecutionsTypeCriteria,
} from "./criteria";
import {
  ExecutionDefaultComparator,
  ExecutionDurationComparator,
} from "./comparators";
...
type GridExecutionFilterCriteria = Exclude<
  ExecutionsFilterCriteria,
  "timePeriod"
>;
type GridExecutionsFilterPairs =
  ExecutionsFilterPairs<GridExecutionFilterCriteria>;
  ...
  implements OnChanges, OnInit, OnDestroy
{
  ...
  gridCriteriaAndComparatorChanged: EventEmitter<GridCriteriaAndComparator> =
    new EventEmitter<GridCriteriaAndComparator>();

  @HostBinding("attr.data-cy") public readonly attributeDataCy =
  ...
  ngOnChanges(changes: SimpleChanges): void {
    if (
      ...
      !CollectionsUtil.isEqual(
        changes["jobExecutions"].previousValue,
        changes["jobExecutions"].currentValue,
      )
    ) {
  ...
  ngOnInit(): void {
    this._filterMutationObserver = (changes) => {
      if (
        changes.some(([key]: GridExecutionsSortPairs) =>
          [
            ...GRID_SUPPORTED_EXECUTIONS_FILTER_KEY,
            ...GRID_SUPPORTED_EXECUTIONS_SORT_KEY,
          ].includes(key),
        )
      ) {
    ...
    this.filtersSortManager.registerMutationObserver(
      this._filterMutationObserver,
    );
  }
  ...
  ngOnDestroy(): void {
    if (
      CollectionsUtil.isNumber(this._gridCriteriaAndComparatorEmitterTimeoutRef)
    ) {
      clearTimeout(this._gridCriteriaAndComparatorEmitterTimeoutRef);
    }

    this.filtersSortManager.deleteMutationObserver(
      this._filterMutationObserver,
    );
  }
  ...
  private _populateManagerFilters(state: ClrDatagridStateInterface): void {
    ...
    if (CollectionsUtil.isArray(state.filters)) {
      if (state.filters.length > 0) {
        const newFilterPairs: GridExecutionsFilterPairs[] = state.filters.map(
          (filter: ExecutionsGridFilter) =>
            [filter.property, filter.value] as GridExecutionsFilterPairs,
        );

        // remove known filters if they are already set in the manager but are missing from grid state
        const filtersForDeletion: GridExecutionsFilterPairs[] =
          GRID_SUPPORTED_EXECUTIONS_FILTER_KEY.filter(
            (supportedCriteria) =>
              this.filtersSortManager.hasFilter(supportedCriteria) &&
              newFilterPairs.findIndex(
                ([criteria]) => supportedCriteria === criteria,
              ) === -1,
          ).map((supportedCriteria) => [supportedCriteria, null]);

        newFilterPairs.push(...filtersForDeletion);

        this.filtersSortManager.bulkUpdate(
          newFilterPairs.map(([criteria, value]) => [
            criteria,
            value,
            "filter",
          ]),
    ...
    if (this.filtersSortManager.hasAnyFilter()) {
      ...
      const filtersForDeletion = GRID_SUPPORTED_EXECUTIONS_FILTER_KEY.filter(
        (criteria) => this.filtersSortManager.hasFilter(criteria),
      ).map((criteria) => [criteria, null] as GridExecutionsFilterPairs);

      if (filtersForDeletion.length > 0) {
        this.filtersSortManager.bulkUpdate(
          filtersForDeletion.map(([criteria, value]) => [
            criteria,
            value,
            "filter",
          ]),
  ...
  private _populateManagerSort(state: ClrDatagridStateInterface): void {
    ...
    if (CollectionsUtil.isDefined(state.sort)) {
      const property: ExecutionsSortCriteria =
        CollectionsUtil.isStringWithContent(state.sort.by)
          ? (state.sort.by as ExecutionsSortCriteria)
          : (state.sort.by as unknown as { property: ExecutionsSortCriteria })
              ?.property;
      const direction = state.sort.reverse
      ...
      const sortsForDeletion: ExecutionsSortPairs[] =
        GRID_SUPPORTED_EXECUTIONS_SORT_KEY.filter(
          (supportedCriteria) =>
            this.filtersSortManager.hasSort(supportedCriteria) &&
            newSortPairs.findIndex(
              ([criteria]) => supportedCriteria === criteria,
            ) === -1,
        ).map((supportedCriteria) => [supportedCriteria, null]);
      ...
      this.filtersSortManager.bulkUpdate(
        newSortPairs.map(([criteria, value]) => [criteria, value, "sort"]),
      );
    ...
    if (this.filtersSortManager.hasAnySort()) {
      ...
      const sortsForDeletion = GRID_SUPPORTED_EXECUTIONS_SORT_KEY.filter(
        (criteria) => this.filtersSortManager.hasSort(criteria),
      ).map((criteria) => [criteria, null] as ExecutionsSortPairs);

      if (sortsForDeletion.length > 0) {
        this.filtersSortManager.bulkUpdate(
          sortsForDeletion.map(([criteria, value]) => [
            criteria,
            value,
            "sort",
          ]),
  ...
  private _paginateExecutions(state: ClrDatagridStateInterface): void {
    ...
    const pageSize = CollectionsUtil.isDefined(this.paginationPageSize)
      ? this.paginationPageSize
      : 10;
    const pageNumber = CollectionsUtil.isDefined(this.paginationPageNumber)
  ...
  private _evaluateGridStateMutation(skipDebouncing = false): void {
    if (
      this._previousAppliedGridState.filter.length !==
    ...
    if (
      ...
      if (
        !CollectionsUtil.isEqual(
          this._previousAppliedGridState.filter,
          this._appliedGridState.filter,
        )
      ) {
        this._emitGridCriteriaAndComparator(skipDebouncing);
    ...
    if (
      !CollectionsUtil.isEqual(
        this._previousAppliedGridState.sort,
        this._appliedGridState.sort,
      )
    ) {
      this._emitGridCriteriaAndComparator(skipDebouncing);
  ...
  private _createFilterCriteria(): Criteria<GridDataJobExecution> {
    ...
    for (const filterPair of this._appliedGridState.filter) {
      ...
      if (filterPair[0] === "startTime") {
        criteria.push(
          new ExecutionsStringCriteria("startTimeFormatted", filterPair[1]),
        );
      ...
      if (filterPair[0] === "endTime") {
        criteria.push(
          new ExecutionsStringCriteria("endTimeFormatted", filterPair[1]),
        );
  ...
  private _createSortComparator(): Comparator<GridDataJobExecution> {
    if (CollectionsUtil.isDefined(this._appliedGridState.sort)) {
      ...
      if (sortCriteria === "duration") {
        return new ExecutionDurationComparator(
          sortValue === ClrDatagridSortOrder.ASC ? "ASC" : "DESC",
        );
      }

      return new ExecutionDefaultComparator(
        sortCriteria,
        sortValue === ClrDatagridSortOrder.ASC ? "ASC" : "DESC",
      );
    }
