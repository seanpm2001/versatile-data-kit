 ...
 */
export class ExecutionsTimePeriodCriteria
  implements Criteria<GridDataJobExecution>
{
  ...
  meetCriteria(executions: GridDataJobExecution[]): GridDataJobExecution[] {
    return [...(executions ?? [])].filter((execution) => {
      if (
        CollectionsUtil.isNil(this._fromDateTime) ||
        CollectionsUtil.isNil(this._toDateTime)
      ) {
        return true;
      }
      ...
      return (
        this._fromDateTime.getTime() <= startTime &&
        startTime <= this._toDateTime.getTime()
      );
    });
  }
}
