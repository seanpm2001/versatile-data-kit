    ...
    OnDestroy
{
  ...
  timeFilterChange(selectedPeriod: SelectedDateTimePeriod): void {
    if (
      this.selectedPeriod.from === selectedPeriod.from &&
      this.selectedPeriod.to === selectedPeriod.to
    ) {
      return;
    }
    ...
    this.isPeriodSelected =
      this.selectedPeriod.from !== null && this.selectedPeriod.to !== null;
  ...
  private _initialize(state: RouteState): void {
    const teamParamKey =
      state.getData<DataPipelinesRouteData["teamParamKey"]>("teamParamKey");
  ...
  private _initializeFiltersSortManager(state: RouteState): void {
    ...
    if (!this.filtersSortManager.hasAnySort()) {
      this.filtersSortManager.setSort(
        SORT_START_TIME_KEY,
  ...
  private _filterExecutions(): void {
    ...
    if (
      CollectionsUtil.isDefined(this.selectedPeriod.from) &&
      CollectionsUtil.isDefined(this.selectedPeriod.to)
    ) {
      timePeriodCriteria = new ExecutionsTimePeriodCriteria(
        this.selectedPeriod.from,
        this.selectedPeriod.to,
      );
      // execute filter by time period
      executionsFilteredAndSorted = timePeriodCriteria.meetCriteria(
        this.jobExecutions,
      );
    } else {
    ...
    if (this.gridCriteriaAndComparator) {
      if (this.gridCriteriaAndComparator.filter) {
        executionsFilteredAndSorted =
          this.gridCriteriaAndComparator.filter.meetCriteria(
            executionsFilteredAndSorted,
          );
      }
      ...
      if (this.gridCriteriaAndComparator.sort) {
        executionsFilteredAndSorted = executionsFilteredAndSorted.sort(
          this.gridCriteriaAndComparator.sort.compare.bind(
            this.gridCriteriaAndComparator.sort,
          ) as (a: GridDataJobExecution, b: GridDataJobExecution) => number,
        );
      }
    }
