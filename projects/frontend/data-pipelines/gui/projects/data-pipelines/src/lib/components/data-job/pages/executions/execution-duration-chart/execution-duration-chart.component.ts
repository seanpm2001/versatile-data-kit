...
import {
  DATA_PIPELINES_DATE_TIME_FORMAT,
...
export class ExecutionDurationChartComponent implements OnInit, OnChanges {
   ...
   */
  @Output() zoomPeriodChanged: EventEmitter<ZoomPeriod> =
    new EventEmitter<ZoomPeriod>();
          ...
          zoom: {
            zoom: {
              ...
              onZoomComplete: (context) => {
                const from = new Date(
                  Math.floor(context.chart.scales["x"].min),
                );
                const to = new Date(Math.ceil(context.chart.scales["x"].max));

                if (
                  this._zoomPeriod.from === from &&
                  this._zoomPeriod.to === to
                ) {
                  return;
                }
    ...
    return this.jobExecutions
      .map((execution) => {
        return {
          ...
          duration:
            Math.round(
              (this._getJobDurationSeconds(execution) / divider) * 100,
            ) / 100,
  ...
  private _emitFocussedExecutionId(activeElements: ActiveElement[]): void {
    if (activeElements.length > 0) {
      const element: { $context?: { raw?: CustomChartData } } =
        activeElements[0].element as unknown;
  ...
  private _getMinMaxExecutionTuple(
    ...
    if (chartData.length === 0) {
      if (
        CollectionsUtil.isDate(this._zoomPeriod.from) &&
        CollectionsUtil.isDate(this._zoomPeriod.to)
      ) {
    ...
    if (chartData.length === 1) {
      if (
        ...
        if (
          this._zoomPeriod.to.getTime() - this._zoomPeriod.from.getTime() >
          5 * this._getTimeUnitMilliseconds("minute")
        ) {
          return [
            this._zoomPeriod.from.getTime(),
            this._zoomPeriod.to.getTime(),
          ];
        }
      }
