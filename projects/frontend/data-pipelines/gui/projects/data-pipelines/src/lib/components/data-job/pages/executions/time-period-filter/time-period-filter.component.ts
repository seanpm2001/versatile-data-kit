...
export class TimePeriodFilterComponent implements OnInit, OnChanges, OnDestroy {
  ...
  @Input() filtersSortManager: Readonly<
    FiltersSortManager<ExecutionsFilterCriteria, string>
  >;
   ...
   */
  @Output() filterChanged: EventEmitter<DateTimePeriod> =
    new EventEmitter<DateTimePeriod>();
  ...
  onDateTimeChange($event: CalendarValue, type: "from" | "to"): void {
    ...
    if (type === "from") {
      if (
        emittedDate.isBefore(this.fromDateTimeMin) ||
        emittedDate.isAfter(this.fromDateTimeMax)
      ) {
        return;
      }
      ...
      this.fromDateTime = new Date(
        this._adjustDateTime(emittedDate, "from", "timezone").valueOf(),
      );
    } else {
      if (
        emittedDate.isBefore(this.toDateTimeMin) ||
        emittedDate.isAfter(this.toDateTimeMax)
      ) {
        return;
      }
      ...
      this.toDateTime = new Date(
        this._adjustDateTime(emittedDate, "to", "timezone").valueOf(),
      );
    }
  }
  ...
  clearFilter(
    $event: MouseEvent,
    triggerMinMaxDateTimeChangeDetection = false,
  ): void {
    $event?.preventDefault();
  ...
  private _updateFiltersSortManager(): void {
    if (
      CollectionsUtil.isNil(this.fromDateTime) ||
      CollectionsUtil.isNil(this.toDateTime)
    ) {
      this._previousSelectedPeriodSerialized = undefined;
      this.filtersSortManager.deleteFilter(FILTER_TIME_PERIOD_KEY);
    } else {
      const serializedDateTimePeriod =
        this._serializeDateTimePeriodPairValues();

      this._previousSelectedPeriodSerialized = serializedDateTimePeriod;
      this.filtersSortManager.setFilter(
        FILTER_TIME_PERIOD_KEY,
        serializedDateTimePeriod,
      );
    }
  }
  ...
  private _changeDetectionSelectedPeriodSerialized(): void {
    if (
      this._previousSelectedPeriodSerialized === this.selectedPeriodSerialized
    ) {
      return;
    }

    if (
      !CollectionsUtil.isString(this.selectedPeriodSerialized) ||
      this.selectedPeriodSerialized.length === 0
    ) {
      if (
        CollectionsUtil.isDefined(this.fromDateTime) ||
        CollectionsUtil.isDefined(this.toDateTime)
      ) {
        this.clearFilter(null);
      }
    } else {
      const deserializedPeriodValues: DateTimePeriod =
        this._deserializeDateTimePeriodPairValues(
          this.selectedPeriodSerialized,
        );
  ...
  private _changeDetectionSelectedPeriod(): void {
    ...
    if (
      this.fromDateTime === this.selectedPeriod.from &&
      this.toDateTime === this.selectedPeriod.to
    ) {
      return;
    }

    if (
      CollectionsUtil.isNil(this.selectedPeriod.from) ||
      CollectionsUtil.isNil(this.selectedPeriod.to)
    ) {
      if (
        CollectionsUtil.isDefined(this.fromDateTime) ||
        CollectionsUtil.isDefined(this.toDateTime)
      ...
      const periodFromAdjusted = this._adjustDateTime(
        this.selectedPeriod.from,
        null,
      );
      const periodToAdjusted = this._adjustDateTime(
        this.selectedPeriod.to,
        null,
      );
      ...
      if (
        CollectionsUtil.isDefined(this.fromDateTimeMin) &&
        CollectionsUtil.isDefined(this.toDateTimeMax)
      ) {
        if (
          periodFromAdjusted.isSameOrAfter(this.fromDateTimeMin) &&
          periodFromAdjusted.isBefore(this.toDateTimeMax)
        ) {
          this.fromDateTime = new Date(
        ...
        if (
          periodToAdjusted.isAfter(this.fromDateTimeMin) &&
          periodToAdjusted.isSameOrBefore(this.toDateTimeMax)
        ) {
          this.toDateTime = new Date(
            this._adjustDateTime(
              this.selectedPeriod.to,
  ...
  private _changeDetectionMinDateTime(): void {
    ...
    if (!this._initiallySetMin) {
      ...
      if (
        CollectionsUtil.isDate(this.fromDateTime) &&
        CollectionsUtil.isDate(this.minDateTime)
      ) {
      ...
      this.fromDateTimeMin = this._adjustDateTime(
        this.minDateTime,
        "min",
        "utc",
      );
      ...
      this.toDateTimeMin = this._adjustDateTime(
        this.fromDateTime ?? this.minDateTime,
        "min",
        "utc",
      );
  ...
  private _changeDetectionMaxDateTime() {
    ...
    if (!this._initiallySetMax) {
      ...
      this.fromDateTimeMax = this._adjustDateTime(
        this.toDateTime,
        "max",
        "utc",
      );
  ...
  private _deserializeDateTimePeriodPairValues(
    dateTimePeriodValues: string,
  ): DateTimePeriod {
    ...
    if (
      CollectionsUtil.isStringWithContent(fromToDateTimeTuple[0]) &&
      /\d+/.test(fromToDateTimeTuple[0])
    ) {
      const parsedFromEpochTime = parseInt(fromToDateTimeTuple[0], 10);

      if (
        CollectionsUtil.isNumber(parsedFromEpochTime) &&
        !CollectionsUtil.isNaN(parsedFromEpochTime)
      ) {
    ...
    if (
      CollectionsUtil.isStringWithContent(fromToDateTimeTuple[1]) &&
      /\d+/.test(fromToDateTimeTuple[1])
    ) {
      const parsedToEpochTime = parseInt(fromToDateTimeTuple[1], 10);

      if (
        CollectionsUtil.isNumber(parsedToEpochTime) &&
        !CollectionsUtil.isNaN(parsedToEpochTime)
      ) {
    ...
    if (
      CollectionsUtil.isDate(fromDateTime) &&
      CollectionsUtil.isDate(toDateTime)
    ) {
      const fromDateTimeMoment = this._adjustDateTime(
        fromDateTime,
        null,
        "utc",
      );
      ...
      if (
        CollectionsUtil.isDefined(this.fromDateTimeMin) &&
        CollectionsUtil.isDefined(this.fromDateTimeMax)
      ) {
        if (
          !fromDateTimeMoment.isBetween(
            this.fromDateTimeMin,
            this.fromDateTimeMax,
          )
        ) {
          return null;
        }
      ...
      if (
        CollectionsUtil.isDefined(this.toDateTimeMin) &&
        CollectionsUtil.isDefined(this.toDateTimeMax)
      ) {
        if (
          !toDateTimeMoment.isBetween(this.toDateTimeMin, this.toDateTimeMax)
        ) {
          return null;
        }
      }
  ...
  private _normalizeUrlAfterLoad(): void {
    ...
    if (
      this.filtersSortManager.filterCriteria[FILTER_TIME_PERIOD_KEY] ===
      this._serializeDateTimePeriodPairValues()
    ) {
      return;
    }
  ...
  private _updateForm(partial: "fromDateTime" | "toDateTime" | "both"): void {
    if (partial === "both" || partial === "fromDateTime") {
      this.tmForm
        .get("fromDateTime")
        .patchValue(
          this._adjustDateTime(this.fromDateTime, "from", "utc").format(
    ...
    if (partial === "both" || partial === "toDateTime") {
      this.tmForm
        .get("toDateTime")
        .patchValue(
          this._adjustDateTime(this.toDateTime, "to", "utc").format(
