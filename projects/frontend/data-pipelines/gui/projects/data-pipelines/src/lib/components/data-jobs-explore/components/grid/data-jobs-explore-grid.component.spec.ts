...
describe("DataJobsExploreGridComponent", () => {
  ...
  beforeEach(() => {
    ...
    dataJobsApiServiceStub.getJobs.and.returnValue(
      of({
        data: {
          content: [],
          totalItems: 0,
  ...
  describe("urlStateManager", () => {
    ...
    it("should verify will invoke default urlStateManager (locally created)", () => {
      ...
      expect(setQueryParamSpy.calls.argsFor(2)).toEqual([
        "description",
        undefined,
        3,
      ]);
      expect(setQueryParamSpy.calls.argsFor(3)).toEqual([
        "deploymentStatus",
        "all",
        4,
      ]);
      expect(setQueryParamSpy.calls.argsFor(4)).toEqual([
        "deploymentLastExecutionStatus",
        undefined,
        5,
      ]);
      expect(setQueryParamSpy.calls.argsFor(5)).toEqual([
        QUERY_PARAM_SEARCH,
        "search",
        0,
      ]);
    ...
    it("should verify will invoke default urlStateManager (dependency injection is null or undefined)", () => {
      ...
      expect(setQueryParamSpy.calls.argsFor(11)).toEqual([
        QUERY_PARAM_SEARCH,
        "search test value 2",
        0,
      ]);
  ...
  describe("viewJobDetails", () => {
    ...
    it("opens viewJobDetails for valid job", () => {
      // Given
      const navigateToSpy = spyOn(component, "navigateTo").and.returnValue(
