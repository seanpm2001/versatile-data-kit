...
export class AppConfigService {
  ...
  loadAppConfig(): Promise<void> {
    return firstValueFrom(
      this.httpClient.get<AppConfig>("/assets/data/appConfig.json"),
    ).then((data) => {
      this.appConfig = data;
      try {
  ...
  getAuthCodeFlowConfig(): AuthConfig {
    ...
    authCodeFlowConfig.redirectUri = replaceWindowLocationOrigin(
      authCodeFlowConfig?.redirectUri,
    );
    authCodeFlowConfig.silentRefreshRedirectUri = replaceWindowLocationOrigin(
