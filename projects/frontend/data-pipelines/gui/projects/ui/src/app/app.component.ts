...
export class AppComponent implements OnInit {
  ...
  private initTokenRefresh() {
    ...
    timer(
      refreshTokenConfig.start,
      AppComponent.toMillis(refreshTokenConfig.checkInterval),
    ).subscribe(() => {
      const remainiTimeMillis =
        this.oauthService.getAccessTokenExpiration() - Date.now();
      if (
        remainiTimeMillis <=
        AppComponent.toMillis(refreshTokenConfig.remainingTime)
      ) {
  ...
  private setCustomTokenAttributes(
    ...
    const consoleCloudUrl =
      this.appConfigService.getConfig().auth.consoleCloudUrl;
  ...
  private getOrgLinkFromQueryParams(defaultOrg: { refLink: string }): string {
    ...
    if (orgLinkBase || orgLinkUnderscored) {
      ...
      return defaultOrg
        ? defaultOrg.refLink
        : this.appConfigService.getConfig().auth.orgLinkRoot;
    }
  }
