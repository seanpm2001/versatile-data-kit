...
describe("generateErrorCode", () => {
  it("should verify will return correct value", () => {
    ...
    expect(errorCode).toEqual(
      `${className}_${publicName}_${methodName}_${additionalDetails}`,
    );
  });
  ...
  it("should verify will generate random string if className is not provided", () => {
    ...
    const spy = spyOn(CollectionsUtil, "generateRandomString").and.returnValue(
      "ClassName1",
    );
    ...
    // Then
    expect(errorCode).toEqual(
      `ClassName1_${publicName}_${methodName}_${additionalDetails}`,
    );
