  ...
  describe("Methods::()", () => {
    ...
    describe("|compare|", () => {
      it("should verify will return -1 and log warn to console", () => {
        ...
        expect(consoleWarnSpy).toHaveBeenCalledWith(
          "PredicatesComparable, unsupported comparison.",
        );
      });
    });
    ...
    describe("|like|", () => {
      it("should verify will return true if one or more predicates return true, otherwise false", () => {
        // Given
        const predicate1EvaluateSpy = spyOn(
          predicate1,
          "evaluate",
        ).and.returnValues(false, false);
        const predicate2EvaluateSpy = spyOn(
          predicate2,
          "evaluate",
        ).and.returnValues(true, false);
        const predicate3EvaluateSpy = spyOn(
          predicate3,
          "evaluate",
        ).and.returnValues(false);
        const comparableStub = new ComparableStub("D");
        predicatesComparable = PredicatesComparable.of(
          predicate1,
          predicate2,
          predicate3,
        );
        ...
        // Then
        expect(r1).toBeTrue();
        expect(predicate1EvaluateSpy.calls.argsFor(0)).toEqual([
          injectedComparable,
        ]);
        expect(predicate2EvaluateSpy.calls.argsFor(0)).toEqual([
          injectedComparable,
        ]);

        expect(r2).toBeFalse();
        expect(predicate1EvaluateSpy.calls.argsFor(1)).toEqual([
          comparableStub,
        ]);
        expect(predicate2EvaluateSpy.calls.argsFor(1)).toEqual([
          comparableStub,
        ]);
        expect(predicate3EvaluateSpy.calls.argsFor(0)).toEqual([
          comparableStub,
        ]);
    ...
    describe("|equal|", () => {
      it("should verify will return true if all predicates return true, otherwise false", () => {
        ...
        const predicate1EvaluateSpy = spyOn(
          predicate1,
          "evaluate",
        ).and.returnValues(true, false, true);
        const predicate2EvaluateSpy = spyOn(
          predicate2,
          "evaluate",
        ...
        expect(r3).toBeTrue();
        expect(predicate1EvaluateSpy.calls.argsFor(2)).toEqual([
          comparableStub2,
        ]);
        expect(predicate2EvaluateSpy.calls.argsFor(1)).toEqual([
          comparableStub2,
    ...
    describe("|lessThanInclusive|", () => {
      it("should verify will return -1 and log warn to console", () => {
        // When
        const comparison =
          predicatesComparable.lessThanInclusive(injectedComparable);
    ...
    describe("|greaterThanInclusive|", () => {
      it("should verify will return -1 and log warn to console", () => {
        // When
        const comparison =
          predicatesComparable.greaterThanInclusive(injectedComparable);
