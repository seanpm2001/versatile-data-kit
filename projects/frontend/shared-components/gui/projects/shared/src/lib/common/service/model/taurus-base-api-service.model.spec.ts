...
} from "./taurus-base-api-service.model";

@Injectable()
class DummyApiService
  extends TaurusBaseApiService<DummyApiService>
...
describe("TaurusBaseApiService", () => {
  ...
  beforeEach(() => {
    spyForRegisterErrorCodes = spyOn(
      DummyApiService.prototype,
      "registerErrorCodes",
    ).and.callThrough();
  ...
  describe("Properties::", () => {
    describe("|errorCodes|", () => {
      ...
      it("should verify values for every public key as provided by intellisense", () => {
        // Then
        const methods: Array<keyof ErrorCodes<DummyApiService>> = [
          "loadData",
          "modifyAssets",
        ];
        ...
        for (const method of methods) {
          const numberOfAvailableErrorCodesPerMethod = Object.keys(
            service.errorCodes[method],
          ).length;
          ...
          for (const [key, code] of additionalDetails) {
            ...
            expect(service.errorCodes[method][key]).toEqual(
              generateErrorCode(
                DummyApiService.CLASS_NAME,
                DummyApiService.PUBLIC_NAME,
                method,
                code,
              ),
            );
    ...
    describe("|objectUUID|", () => {
      it("should verify value is DummyApiService", () => {
        // Given
        spyOn(CollectionsUtil, "generateObjectUUID").and.callFake(
          (value) => value,
        );
  ...
  describe("Methods::", () => {
    describe("|registerErrorCodes|", () => {
      ...
      it("should verify thrown error will log in console", () => {
        // Given
        spyOn(CollectionsUtil, "isFunction").and.throwError(
        ...
        expect(consoleSpy).toHaveBeenCalledWith(
          "Cannot register Service Error Codes!",
        );
      });
    });
  });
