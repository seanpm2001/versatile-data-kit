...
export class VdkCopyToClipboardButtonComponent implements OnInit {
  ...
  isSafari: boolean = /^((?!chrome|android).)*safari/i.test(
    navigator.userAgent,
  );
  ...
  constructor(
    ...
    this.translateService.loadTranslationsForComponent(
  ...
  ngOnInit() {
    ...
    this.copyAlert =
      this.copyAlert ||
      this.translateService.translate("copy-to-clipboard-button.copied");
  }
