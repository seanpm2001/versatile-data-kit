...
class PageObject {
  ...
  populateLocators(): void {
    this.fixtureElement = fixture.debugElement.query(
      By.css("vdk-form-section-container"),
    );
  }
...
@Component({
  template: ` <form
    ...
    (ngSubmit)="doSubmit(editProfileForm.value)"
  >
    <vdk-form-section-container
      [isSubmitEnabled]="false"
      [formState]="formState"
      (sectionState)="sectionState($event)"
    >
...
describe("Form Section Container Component", () => {
  ...
  it("should readOnly and Edit sections to be toggled on Edit", () => {
    expect(page.getReadOnlySectionElements().length).toBe(
      1,
      "Readonly Section shown initially",
    );
    expect(page.getEditSectionElements().length).toBe(
      0,
      "Edit section hidden initially",
    );
    expect(el.querySelectorAll(".csp-edit-button").length).toBe(
      1,
      "Edit button shown initially",
    );
    const editButton: HTMLInputElement = <HTMLInputElement>(
      el.querySelector(".csp-edit-button")
    );
    editButton.click();
    fixture.detectChanges();
    ...
    expect(page.getReadOnlySectionElements().length).toBe(
      0,
      "Readonly section hidden on Edit button",
    );
    expect(page.getEditSectionElements().length).toBe(
      1,
      "Edit section shown on Edit button",
    );
  });
  ...
  it("should readOnly and Edit sections to be toggled on Cancel", () => {
    ...
    expect(el.querySelectorAll(".csp-cancel-button").length).toBe(
      1,
      "Cancel button to be shown",
    );
    const cancelButton: HTMLInputElement = <HTMLInputElement>(
      el.querySelector(".csp-cancel-button")
    );
    cancelButton.click();
    fixture.detectChanges();
    ...
    expect(page.getReadOnlySectionElements().length).toBe(
      1,
      "Readonly section shown on Cancel",
    );
    expect(page.getEditSectionElements().length).toBe(
      0,
      "Edit section hidden on Cancel",
    );
  });
  ...
  it("should show Readonly and Edit section after save", () => {
    ...
    expect(el.querySelectorAll(".csp-save-button").length).toBe(
      1,
      "Save button shown",
    );
    const saveButton: HTMLInputElement = <HTMLInputElement>(
      el.querySelector(".csp-save-button")
    );
    expect(page.getSaveBtnSpan().innerText.trim()).toBe(
      "Save",
      "save btn shown",
    );
    saveButton.click();
