...
export class VdkFormState {
  ...
  constructor(
    _state: FORM_STATE,
    _sectionsToInclude?: string[],
    _emittingSection?: string,
  ) {
    this.state = _state;
...
export class VdkFormSectionContainerComponent {
  ...
  @Input("formState") set formState(_formState: VdkFormState) {
    ...
    if (
      (_formState.emittingSection &&
        _formState.emittingSection !== this.sectionName) ||
      (!_formState.emittingSection &&
        _formState.sectionsToInclude.length === 0) ||
