...
describe("VdkSimpleTranslateService", () => {
  ...
  it("should translate string as per specified language", () => {
    ...
    expect(service.translate("test.return-to-chat")).toEqual(
      "Retornar a conversacion",
    );
  });
  ...
  it("error message should show if key is not found", () => {
    ...
    expect(service.translate("test.some-non-existing-key")).toEqual(
      "!! Key test.some-non-existing-key not found !!",
    );
  });
});
