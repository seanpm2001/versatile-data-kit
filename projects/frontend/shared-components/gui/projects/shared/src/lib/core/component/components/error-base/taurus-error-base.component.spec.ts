...
class TaurusErrorBaseComponentStub extends TaurusErrorBaseComponent {
  ...
  static override readonly PUBLIC_NAME: string =
    "Taurus-Error-Base-Component-Stub";
...
describe("TaurusErrorBaseComponent", () => {
  let randomServiceStub: jasmine.SpyObj<{
    load: () => void;
    errorCodes: ErrorCodes<{ load: () => void }>;
  }>;
  ...
  beforeEach(() => {
    randomServiceStub = jasmine.createSpyObj<{
      load: () => void;
      errorCodes: ErrorCodes<{ load: () => void }>;
    }>("randomServiceStub", ["load"]);
  ...
  describe("Angular lifecycle hooks::", () => {
    describe("|ngOnDestroy|", () => {
      it("should verify will dispose error records in error store", () => {
        // Given
        const spyErrorStoreDispose = spyOn(
          component.errors,
          "dispose",
        ).and.callThrough();
  ...
  describe("Methods::", () => {
    describe("|generateErrorCode|", () => {
      it("should verify will return error code", () => {
        // When
        // @ts-ignore
        const errorCode = component.generateErrorCode(
          "ClassName",
          "Public-Name",
          "methodName",
          "unknown",
    ...
    describe("|processServiceRequestError|", () => {
      it("should verify will process error and record in local store", () => {
        ...
        // When
        // @ts-ignore
        component.processServiceRequestError(
          randomServiceStub.errorCodes.load,
          error,
        );
