 ...
 */
@Directive()
export class TaurusErrorBaseComponent
  ...
  implements OnDestroy
{
   ...
   */
  protected processServiceRequestError(
    serviceHttpErrorCodes: Record<keyof ServiceHttpErrorCodes, string>,
    error: unknown,
  ): void {
    this.errors.record(
      processServiceRequestError(this.objectUUID, serviceHttpErrorCodes, error),
    );
  }
}
