...
export abstract class TaurusBaseComponent
  extends TaurusErrorBaseComponent
  implements OnInit, OnDestroy
{
    ...
    if (model.status === FAILED) {
      const previousErrorRecords: ErrorRecord[] =
        previousModel instanceof ComponentModel
          ? previousModel.getComponentState().errors.records
          : [];
      const distinctErrorRecordsFromPreviousCycle = model
        .getComponentState()
        .errors.distinctErrorRecords(previousErrorRecords);
      ...
      if (
        !TaurusBaseComponent._executeTaurusComponentHook(
          ...
          "onModelError",
          model,
          distinctErrorRecordsFromPreviousCycle,
        )
      ) {
        TaurusBaseComponent._executeTaurusComponentHook(
          this,
  ...
  protected setStateIdle(): void {
    this.componentService.idle(
      this.model.prepareForDestroy().getComponentState(),
    );
  }
