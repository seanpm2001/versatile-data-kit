  ...
  describe("Methods::", () => {
    ...
    describe("|withError|", () => {
      it("should verify will invoke correct method with correct payload", () => {
        ...
        const updateSpy = spyOn(
          model.getComponentState().errors,
          "record",
        ).and.callThrough();
    ...
    describe("|removeErrorCode|", () => {
      it("should verify will invoke correct method with correct payload", () => {
        ...
        const errorStoreStub = jasmine.createSpyObj<ErrorStore>(
          "errorStoreStub",
          ["removeCode"],
        );
        const spyComponentState = spyOn(
          model,
          "getComponentState",
        ).and.returnValue(
          ComponentStateImpl.of({
        ...
        // When
        const ref = model.removeErrorCode(
          errorCodes[0],
          errorCodes[1],
          errorCodes[2],
        );
        ...
        expect(errorStoreStub.removeCode).toHaveBeenCalledWith(
          errorCodes[0],
          errorCodes[1],
          errorCodes[2],
        );
    ...
    describe("|removeErrorCodePatterns|", () => {
      it("should verify will invoke correct method with correct payload", () => {
        // Given
        const errorCodePatterns: string[] = [
          "errorCodePattern10",
          "errorCodePattern30",
          "errorCodePattern20",
        ];
        ...
        // When
        const ref = model.removeErrorCodePatterns(
          errorCodePatterns[0],
          errorCodePatterns[1],
          errorCodePatterns[2],
        );
    ...
    describe("|prepareForDestroy|", () => {
      it("should verify will invoke correct method with correct payload", () => {
        // Given
        const spyWithStatusIdle = spyOn(model, "withStatusIdle").and.callFake(
          CallFake,
        );
        const spyUpdate = spyOn(
          model,
