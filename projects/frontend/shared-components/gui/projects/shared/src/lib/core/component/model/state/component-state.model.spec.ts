...
describe("ComponentStateImpl", () => {
  ...
  beforeEach(() => {
    const apiPredicate1: ApiPredicate = {
      sort: ASC,
      property: "test.property.10",
      pattern: "test.pattern.20",
    };
    const apiPredicate2: ApiPredicate = {
      sort: ASC,
      property: "test.property.30",
      pattern: "test.pattern.40",
    };
    ...
    const errorRecords: ErrorRecord[] = [
      {
        ...
        objectUUID: CollectionsUtil.generateObjectUUID("RandomClassName"),
    ...
    partialStateMock = {
      ...
      status: LOADED,
      navigationId: 11,
      errors: ErrorStoreImpl.of(errorRecords.map((r) => ({ ...r }))),
      search: "testSearch",
  ...
  it("should verify provided value will be correctly assigned", () => {
    ...
    expect(instance.errors).toEqual(partialStateMock.errors);
  ...
  it("should verify will correctly assign default values", () => {
    // When
    const instances: ComponentState[] = [
      new ComponentStateImpl({}),
      new ComponentStateImpl(undefined),
    ];
    ...
    for (const instance of instances) {
      ...
      expect(instance.errors).toEqual(ErrorStoreImpl.empty());
    ...
    describe("Methods::", () => {
      describe("|of|", () => {
        it("should verify factory method will create instance", () => {
          ...
          expect(instance.search).toEqual(partialStateMock.search);
          expect(instance.routePath).toEqual(partialStateMock.routePath);
          expect(instance.routePathSegments).toBe(
            partialStateMock.routePathSegments,
          );
          expect(instance.page).toBe(partialStateMock.page);
          expect(instance.order).toBe(partialStateMock.order);
          expect(instance.filter).toBe(partialStateMock.filter);
      ...
      describe("|fromLiteralComponentState|", () => {
        it("should verify will create instance of ComponentStateImpl from LiteralComponentState", () => {
          // When
          const instance =
            ComponentStateImpl.fromLiteralComponentState(literalStateMock);

          // Then
          expect(instance).toEqual(stateMock);
          expect(instance.order.criteria[0]).toBe(stateMock.order.criteria[0]);
          expect(instance.filter.criteria[0]).toBe(
            stateMock.filter.criteria[0],
          );
          expect(instance.requestParams.get("test_param.1")).toBe(
            stateMock.requestParams.get("test_param.1"),
          );
          expect(instance.requestParams.get("test_param.2")).toBe(
            stateMock.requestParams.get("test_param.2"),
          );
          expect(instance.uiState.get("test_uiState.1")).toBe(
            stateMock.uiState.get("test_uiState.1"),
          );
          expect(instance.uiState.get("test_uiState.2")).toBe(
            stateMock.uiState.get("test_uiState.2"),
          );
          expect(instance.data.get("test_data.1")).toBe(
            stateMock.data.get("test_data.1"),
          );
          expect(instance.data.get("test_data.2")).toBe(
            stateMock.data.get("test_data.2"),
          );
        });
      });
      ...
      describe("|cloneDeepLiteral|", () => {
        it("should verify will create deep clone from LiteralComponentState", () => {
          // When
          const instance =
            ComponentStateImpl.cloneDeepLiteral(literalStateMock);
          ...
          expect(instance.requestParams["test_param.1"]).not.toBe(
            literalStateMock.requestParams["test_param.1"],
          );
          expect(instance.requestParams["test_param.2"]).not.toBe(
            literalStateMock.requestParams["test_param.2"],
          );
          expect(instance.uiState["test_uiState.1"]).not.toBe(
            literalStateMock.uiState["test_uiState.1"],
          );
          expect(instance.uiState["test_uiState.2"]).not.toBe(
            literalStateMock.uiState["test_uiState.2"],
          );
          expect(instance.data["test_data.1"]).not.toBe(
            literalStateMock.data["test_data.1"],
          );
          expect(instance.data["test_data.2"]).not.toBe(
            literalStateMock.data["test_data.2"],
          );
        });
      });
    });
  ...
  describe("Methods::", () => {
    describe("|toLiteral|", () => {
      it("should verify will create LiteralComponentState", () => {
        ...
        expect(instance.requestParams["test_param.1"]).toBe(
          literalStateMock.requestParams["test_param.1"],
        );
        expect(instance.requestParams["test_param.2"]).toBe(
          literalStateMock.requestParams["test_param.2"],
        );
        expect(instance.uiState["test_uiState.1"]).toBe(
          literalStateMock.uiState["test_uiState.1"],
        );
        expect(instance.uiState["test_uiState.2"]).toBe(
          literalStateMock.uiState["test_uiState.2"],
        );
        expect(instance.data["test_data.1"]).toBe(
          literalStateMock.data["test_data.1"],
        );
        expect(instance.data["test_data.2"]).toBe(
          literalStateMock.data["test_data.2"],
        );
      });
    });
    ...
    describe("|toLiteralCloneDeep|", () => {
      it("should verify will create LiteralComponentState deep cloned", () => {
        // When
        const instance = stateMock.toLiteralCloneDeep();
