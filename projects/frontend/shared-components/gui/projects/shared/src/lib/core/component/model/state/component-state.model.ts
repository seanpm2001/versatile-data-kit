...
export interface ComponentState
  extends Literal<LiteralComponentState>,
    Copy<ComponentState> {
  /**
   * ** Identifier for Component State.
   */
...
export class ComponentStateImpl implements ComponentState {
  ...
  constructor(stateModelProp: Partial<ComponentState>) {
    const stateModel: Partial<ComponentState> = CollectionsUtil.isDefined(
      stateModelProp,
    )
      ? stateModelProp
      : {};
   ...
   */
  static fromLiteralComponentState(
    literalStateModel: LiteralComponentState,
  ): ComponentStateImpl {
    return ComponentStateImpl.of({
      ...
      requestParams: CollectionsUtil.transformObjectToMap(
        literalStateModel.requestParams,
      ),
