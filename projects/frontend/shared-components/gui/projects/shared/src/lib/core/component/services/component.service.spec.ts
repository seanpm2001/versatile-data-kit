...
describe("ComponentService -> ComponentServiceImpl", () => {
  ...
  beforeEach(() => {
    storeStub$ = jasmine.createSpyObj<Store<StoreState>>("store", [
      "select",
      "dispatch",
    ]);
  ...
  describe("Methods::", () => {
      ...
      it("should verify will initialize correct component and dispatch ComponentInit", () => {
        ...
        storeStub$.select.and.callFake((param) => {
          if (param === STORE_COMPONENTS) {
            components = createStoreState(componentState.toLiteral())[
              STORE_COMPONENTS
            ];
        ...
        // Then
        expect(storeStub$.select.calls.argsFor(0)).toEqual([
        ...
        response$.subscribe((model) => {
          ...
          expect(model["_componentState"]).toEqual(
            getComponentState(components, id),
          );
    ...
    describe("|load|", () => {
      it("should verify will dispatch ComponentLoading with provided ComponentState", () => {
        ...
        storeStub$.select.and.returnValue(
          of(
            createStoreState(
              createComponentState(router, id, LOADED).toLiteral(),
            )[STORE_COMPONENTS],
          ),
        );
        ...
        const assertionComponentState = createComponentState(
          router,
          id,
          LOADED,
        );
    ...
    describe("|hasInSegment|", () => {
      it(
        ...
        marbles((m) => {
          ...
          // When
          const response$ = service.hasInSegment("someComponentId", [
            "test_domain/context",
            "test_entity/10",
          ]);
      ...
      it(
        ...
        marbles((m) => {
          ...
          // When
          const response$ = service.hasInSegment("newComponentId", [
            "test_domain/context",
            "test_entity/10",
          ]);
    ...
    describe("|onInit|", () => {
      it(
        ...
        marbles((m) => {
          // Given
          const cA = createStoreState(
            createComponentState(router, "testId10", INITIALIZED).toLiteral(),
          );
          const cB = createStoreState(
            createComponentState(router, "testId10", IDLE).toLiteral(),
          );
          const cC = createStoreState(
            createComponentState(router, "testId10", LOADING).toLiteral(),
          );
          const cD = createStoreState(
            createComponentState(router, "testId10", LOADED).toLiteral(),
          );
          const cE = createStoreState(
            createComponentState(router, "testId22", LOADED).toLiteral(),
          );
          const cF = createStoreState(
            createComponentState(router, "testId33", LOADED).toLiteral(),
          );
    ...
    describe("|onLoaded|", () => {
      it(
        ...
        marbles((m) => {
          // Given
          const cA = createStoreState(
            createComponentState(router, "testId1", INITIALIZED).toLiteral(),
          );
          const cB = createStoreState(
            createComponentState(router, "testId1", IDLE).toLiteral(),
          );
          const cC = createStoreState(
            createComponentState(router, "testId1", LOADING).toLiteral(),
          );
          const cD = createStoreState(
            createComponentState(router, "testId1", LOADED).toLiteral(),
          );
          const cE = createStoreState(
            createComponentState(router, "testId2", LOADED).toLiteral(),
          );
          const cF = createStoreState(
            createComponentState(router, "testId3", LOADED).toLiteral(),
          );
          const componentsMarbleStream$ = m.cold("-a--bc---d--e----f-|", {
            a: cA[STORE_COMPONENTS],
            b: cB[STORE_COMPONENTS],
          ...
          // When
          const response$ = service.onLoaded(
            id,
            router.state.routePathSegments,
          );
      ...
      it(
        ...
        marbles((m) => {
          // Given
          const cA = createStoreState(
            createComponentState(router, id, INITIALIZED).toLiteral(),
          );
          const cB = createStoreState(
            createComponentState(router, id, IDLE).toLiteral(),
          );
          const cC = createStoreState(
            createComponentState(router, id, LOADING).toLiteral(),
          );
      ...
      it(
        ...
        marbles((m) => {
          ...
          routerServiceStub.getState.and.returnValue(
            m.cold("a----", { a: router.state }),
          );
      ...
      it(
        ...
        marbles((m) => {
          ...
          const cD = createStoreState(
            createComponentState(router, id, FAILED).toLiteral(),
          );
          const componentsMarbleStream$ = m.cold("-a--bc---d---", {
            a: cA[STORE_COMPONENTS],
            b: cB[STORE_COMPONENTS],
    ...
    describe("|getModel|", () => {
      it(
        ...
        marbles((m) => {
          // Given
          const cA = createStoreState(
            createComponentState(router, "testId21", INITIALIZED).toLiteral(),
          );
          const cB = createStoreState(
            createComponentState(router, "testId21", IDLE).toLiteral(),
          );
          const cC = createStoreState(
            createComponentState(router, "testId21", LOADING).toLiteral(),
          );
          const cD = createStoreState(
            createComponentState(router, "testId21", LOADED).toLiteral(),
          );
          const cE = createStoreState(
            createComponentState(router, "testId32", LOADED).toLiteral(),
          );
          const cF = createStoreState(
            createComponentState(router, "testId33", LOADED).toLiteral(),
          );
          ...
          // When
          const response$ = service.getModel(
            id,
            router.state.routePathSegments,
          );
    ...
    describe("|dispatchAction|", () => {
      it("should verify will invoke correct methods", () => {
        ...
        const getModelSpy = spyOn(service, "getModel").and.returnValue(
          of(modelStub),
        );
        const actionStub = {
          type: typeStub,
        ...
        const genericActionOfSpy = spyOn(GenericAction, "of").and.returnValue(
          actionStub,
        );
        ...
        expect(getModelSpy).toHaveBeenCalledWith(
          componentState.id,
          componentState.routePathSegments,
          ["*"],
        );
        expect(genericActionOfSpy).toHaveBeenCalledWith(
          typeStub,
          componentState,
          undefined,
        );
...
const createComponentState = (
  router: RouterReducerState<RouteState>,
  id: string,
  status: StatusType,
) => {
...
const getComponentState = (
  state: StoreState | LiteralComponentsState,
  id: string,
): ComponentState => {
  ...
  return ComponentStateImpl.fromLiteralComponentState(
    componentsState.routePathSegments["test_domain/context"].routePathSegments[
      "test_entity/10"
    ].components[id],
  );
};
