...
export function componentReducer(
  ...
  switch (action.type) {
    ...
    case COMPONENT_LOADING:
      actionComponentState = (action as ComponentIdle | ComponentLoading)
        .payload;
    ...
    case COMPONENT_LOADED:
      actionComponentState = (action as ComponentLoaded | ComponentUpdate)
        .payload;
      storeLiteralComponentState = stateHelper.getLiteralComponentState(
        actionComponentState.id,
        actionComponentState.routePathSegments,
      ...
      stateHelper.updateLiteralComponentState({
        ...
        ...actionLiteralComponentState,
        status:
          action instanceof ComponentLoaded
            ? LOADED
            : actionComponentState.status,
        data: getComponentStateData(
          actionLiteralComponentState,
          storeLiteralComponentState,
        ),
      });
    ...
    case ROUTER_NAVIGATION:
      const routeSegments = (action as RouterNavigationAction<RouteState>)
        .payload.routerState.routeSegments;
...
const getComponentStateData = (
  ...
  if (
    CollectionsUtil.isLiteralObjectWithProperties(actionComponentState.data)
  ) {
