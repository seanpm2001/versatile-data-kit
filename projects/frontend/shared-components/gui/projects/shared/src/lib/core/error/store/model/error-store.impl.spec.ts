...
describe("ErrorStoreImpl", () => {
    ...
    describe("Methods::", () => {
      ...
      describe("|fromLiteral|", () => {
        it("should verify will invoke method ErrorStoreImpl.cloneDeepErrorRecords", () => {
          // Given
          const spyCloneDeepErrorRecords = spyOn(
            ErrorStoreImpl,
            "cloneDeepErrorRecords",
          ).and.callThrough();
      ...
      describe("|cloneDeepErrorRecords|", () => {
        it("should verify will clone deep provided ErrorRecords literals", () => {
          // When
          const recordsClonedDeep =
            ErrorStoreImpl.cloneDeepErrorRecords(errorRecords);
  ...
  describe("Methods::", () => {
    ...
    beforeEach(() => {
      ...
      // @ts-ignore
      spyExecuteChangeListeners = spyOn(
        ErrorStoreImpl,
        "_executeChangeListeners",
      ).and.callThrough();
    ...
    describe("|hasCode|", () => {
      ...
      it("should verify will return false if code does not exist", () => {
        ...
        // When
        const value1 = instance1.hasCode(
          "RandomCode",
          "UnknownCode",
          "NotExistedCode",
        );
        const value2 = instance1.hasCode("RandomCode");
        const value3 = instance2.hasCode(
          "RandomCode",
          "UnknownCode",
          "NotExistedCode",
        );
    ...
    describe("|hasCodePattern|", () => {
      ...
      it("should verify will return false if codePattern does not exist", () => {
        ...
        // When
        const value1 = instance1.hasCodePattern(
          "RandomCode",
          undefined,
          "UnknownCode",
          null,
        ...
        const value5 = instance2.hasCodePattern(
          "RandomCode",
          null,
          "NotExistedCode",
          `${errorCodes[1]}$`,
    ...
    describe("|record|", () => {
      it("should verify will record error", () => {
        ...
        instance.record(
          errorRecords[1].code,
          errorRecords[1].objectUUID,
          errorRecords[1].error,
        );
        expect(instance.records).toEqual([
        ...
        instance.record(
          errorRecords[4].code,
          errorRecords[4].objectUUID,
          errorRecords[4].error,
          errorRecords[4].httpStatusCode,
        );
        expect(instance.records).toEqual([
          { ...errorRecords[1], time: dateNow1, httpStatusCode: null },
          { ...errorRecords[0], time: dateNow2 },
        ...
        expect(spyExecuteChangeListeners).toHaveBeenCalledWith(
          instance,
          instance.changeListeners,
        );
      });
    ...
    describe("|findRecordsByPattern|", () => {
      ...
      it("should verify will return Array of ErrorRecord empty or filled with elements if found", () => {
        ...
        // When/Then 2
        const found1 = instance.findRecordsByPattern(
          errorCodes[2],
          undefined,
          errorCodes[1],
          null,
    ...
    describe("|onChange|", () => {
      it("should verify will add listener for change in local repository", () => {
        // Given
        const listener1: (store: ErrorStoreImpl) => void = (
          store: ErrorStoreImpl,
        ) => {
          expect(store).toBe(instance);
        };
        const listener2: (store: ErrorStoreImpl) => void = (
          store: ErrorStoreImpl,
        ) => {
          expect(store).toBe(instance);
        };
      ...
      it("should verify attached listeners are executed on ErrorRecord recording", () => {
        ...
        const listener1 = jasmine
          .createSpy<(_store: ErrorStoreImpl) => void>("listener1")
          .and.callFake(CallFake);
        const listener2 = jasmine
          .createSpy<(_store: ErrorStoreImpl) => void>("listener2")
        ...
        expect(spyConsoleError).toHaveBeenCalledWith(
          `Taurus ErrorStore failed to execute change listeners`,
          error,
        );
      });
    });
    ...
    describe("|equals|", () => {
      ...
      it("should verify will return false when both instances of ErrorStoreImpl have same ErrorRecord size but for one objectUUID is different", () => {
        ...
        instance2.record({
          ...errorRecords[5],
          objectUUID: "newObjectUUID_not_overlap",
        });
        instance2.record(errorRecords[6]);
        instance2.record(errorRecords[7]);
      ...
      it("should verify will return false when both instances of ErrorStoreImpl have same ErrorRecord size but for one error is different v1", () => {
        ...
        instance2.record({
          ...errorRecords[2],
          error: new Error("New Error Thrown not overlap"),
        });
        instance2.record(errorRecords[3]);
        instance2.record(errorRecords[4]);
        instance2.record(errorRecords[5]);
...
describe("filterErrorRecords", () => {
  ...
  it("should verify filters Array of ErrorRecord correctly", () => {
    ...
    // When
    const filteredErrorRecords1 = filterErrorRecords(instance.records, [
      errorCodes[0],
      errorCodes[4],
    ]);
    const filteredErrorRecords2 = filterErrorRecords(
      instance.records,
      [errorCodes[4]],
      ["SomeCdRnd", null],
    );
    const filteredErrorRecords3 = filterErrorRecords(
      instance.records,
      [],
      ["SomeBng$", undefined],
    );
    const filteredErrorRecords4 = filterErrorRecords(
      instance.records,
      null,
      null,
    );
    ...
    // Then
    expect(filteredErrorRecords1).toEqual([
      instance.records[4],
      instance.records[0],
    ]);
    expect(filteredErrorRecords2).toEqual([
      instance.records[4],
      instance.records[2],
      instance.records[1],
    ]);
