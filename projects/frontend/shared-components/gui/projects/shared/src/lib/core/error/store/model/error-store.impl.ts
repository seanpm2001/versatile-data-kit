...
export class ErrorStoreImpl implements ErrorStore {
  ...
  hasCode(...errorCodes: string[]): boolean {
    return errorCodes.some(
      (code) => this.records.findIndex((r) => r.code === code) !== -1,
    );
  }
  ...
  hasCodePattern(...errorCodesPatterns: string[]): boolean {
    try {
      return errorCodesPatterns.some((errorPattern) => {
        ...
        return (
          this.records.findIndex((r) => errorPatternRegex.test(r.code)) !== -1
        );
      });
    } catch (e) {
      console.error(e);
  ...
  record(
    ...
    if (
      CollectionsUtil.isString(param) &&
      CollectionsUtil.isString(objectUUID)
    ) {
      const foundIndex = errorRecordsShallowCopy.findIndex(
        (r) => r.code === param && r.objectUUID === objectUUID,
      );
      ...
      CollectionsUtil.isLiteralObject(param) &&
      CollectionsUtil.isNil(objectUUID)
    ) {
      const foundIndex = errorRecordsShallowCopy.findIndex(
        (r) =>
          r.code === (param as ErrorRecord).code &&
          r.objectUUID === (param as ErrorRecord).objectUUID,
      );
      const errorRecord: ErrorRecord = {
  ...
  removeCode(...errorCodes: string[]): void {
    ...
    try {
      errorRecordsShallowCopy = errorRecordsShallowCopy.filter(
        (r) => !errorCodes.includes(r.code),
      );
    } catch (e) {
      console.error(e);
    }
  ...
  removeCodePattern(...errorCodePatterns: string[]): void {
    ...
    for (const errorPattern of errorCodePatterns) {
      try {
        ...
        errorCodesShallowCopy = errorCodesShallowCopy.filter(
          (r) => !errorPatternRegex.test(r.code),
        );
      } catch (e) {
        console.error(e);
      }
  ...
  distinctErrorRecords(errorRecords: ErrorRecord[]): ErrorRecord[] {
    const _errorRecords: ErrorRecord[] = CollectionsUtil.isArray(errorRecords)
      ? errorRecords
      : [];

    return this.records.filter(
      (r) =>
        _errorRecords.findIndex((rInjected) =>
          ErrorStoreImpl._checkErrorRecordsEquality(r, rInjected),
        ) === -1,
    );
  }
  ...
  equals(obj: ErrorStore): boolean {
    ...
    for (let i = 0; i < this.records.length; i++) {
      if (
        !ErrorStoreImpl._checkErrorRecordsEquality(
          this.records[i],
          obj.records[i],
        )
      ) {
  ...
  private static _checkErrorRecordsEquality(
    errorRecord1: ErrorRecord,
  ...
  private static _executeChangeListeners(
    store: ErrorStoreImpl,
    changeListeners: Array<ErrorStoreChangeListener<ErrorStore>>,
  ): void {
    if (
      !CollectionsUtil.isArray(changeListeners) ||
...
export const filterErrorRecords = (
  ...
  return [...errorRecords]
    ...
    .filter((r) => {
      return (
        (errorCodes ?? []).includes(r.code) ||
        errorPatternsRegex.some((errorPatternRegex) =>
          errorPatternRegex.test(r.code),
        )
      );
    });
