...
import {
  getApiFormattedErrorMessage,
  getHumanReadableStatusText,
...
describe("processServiceRequestError", () => {
    ...
    for (const [
      ...
      it(`${description}`, () => {
        ...
        const spyGenerateObjectUUID = spyOn(
          CollectionsUtil,
          "generateObjectUUID",
        ).and.returnValue("ErrorCodeClass_uuid");

        // When
        const errorRecord = processServiceRequestError(
          objectUUID,
          serviceHttpErrorCodes,
          _error,
        );
        ...
        if (numberOfInvoke > 0) {
          expect(spyGenerateObjectUUID).toHaveBeenCalledWith(
            "UnknownClassName",
          );
        }
      });
  ...
  describe("should verify will return ErrorRecord for usual scenarios when errorCode is", () => {
    ...
    const errors: Partial<
      Record<keyof ServiceHttpErrorCodes, HttpErrorResponse>
    > = {
    ...
    for (const [
      ...
      it(`${_description}`, () => {
        ...
        // When
        const _errorRecord = processServiceRequestError(
          _objectUUID,
          _serviceHttpErrorCodes,
          _error,
        );
