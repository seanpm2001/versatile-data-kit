...
export class NavigationService {
  ...
  _navigationSystemEventHandler_(
    payload: SystemEventNavigatePayload,
  ): Promise<boolean> {
  ...
  navigateTo(
    ...
    return this._navigateWithAction(
      replaceValues,
      activatedRoute,
      "navigateTo",
    );
  ...
  resolveNavigateActionUrl(
    ...
    return this._extractRouteState(activatedRoute)
      .then((state) =>
        this._constructNavigateParameters(state, navigateActionType),
      )
      .then((params) =>
  ...
  private _extractRouteState(
    ...
    if (CollectionsUtil.isDefined(activatedRoute)) {
      ...
      return Promise.resolve(
        this._routeStateFactory.create(
  ...
  private _resolveNavigateUrl(
    ...
    if (dataKey === "redirect" || dataKey === "navigateBack") {
      if (
        navigateAction.path === "$.parent" ||
        navigateAction.path === "$.requested"
      ) {
  ...
  private _resolveNavigateValues(
    ...
    if (CollectionsUtil.isString(path)) {
      ...
      console.error(
        `RouteConfig error! NavigationAction config, "path" property is missing.`,
      );
    }

    for (const replacer of replacers) {
      const replaceValue =
        replaceValues[replacer.replaceValue] ?? replacer.replaceValue;
