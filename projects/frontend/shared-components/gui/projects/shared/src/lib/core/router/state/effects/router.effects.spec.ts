...
describe("RouterEffects", () => {
  ...
  it(
    "should use Location.go()",
    marbles((m) => {
      routeAction$ = m.hot("-a", {
        a: new LocationGo({ path: "/test", query: "search=random", state: {} }),
      });
  ...
  it(
    `should verify will handle error and won't invoke Location.go()`,
    marbles((m) => {
      routeAction$ = m.hot("-#");
  ...
  it(
    `should verify will handle error and won't invoke navigate back`,
    marbles((m) => {
      routeAction$ = m.hot("-#");
  ...
  it(
    `should verify will handle error and won't invoke Location.forward()`,
    marbles((m) => {
      routeAction$ = m.hot("-#");
    ...
    marbles((m) => {
      ...
      effects.routerNavigate$.subscribe(() => {
        expect(routerStub.navigate).toHaveBeenCalledWith(
          ["/domain", "context", "entity", 10, "sub-entity", 7],
  ...
  it(
    `should verify will handle error and won't invoke router navigate`,
    marbles((m) => {
      routeAction$ = m.hot("-#");
