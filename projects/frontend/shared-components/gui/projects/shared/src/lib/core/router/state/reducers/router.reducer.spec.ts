...
describe("routerReducer", () => {
  it("should verify will invoke correct methods", () => {
    ...
    const storedRouterStateStub = jasmine.createSpyObj<RouterState>(
      "routerState",
      ["appendPrevious"],
    );
    const newRouterStateStub = jasmine.createSpyObj<RouterState>(
      "routerState",
      ["appendPrevious"],
    );
    ...
    expect(factoryMethodSpy).toHaveBeenCalledWith(
      navigateAction.payload.routerState,
      navigateAction.payload.event.id,
    );
    expect(newRouterStateStub.appendPrevious).toHaveBeenCalledWith(
      storedRouterStateStub,
    );
  });
});
