    ...
    describe("Methods::()", () => {
      describe("|post|", () => {
        it("should verify will post NON-BLOCKING System Event to 2 Handlers", fakeAsync(() => {
          // Given
          const spyHandler1: jasmine.SpyObj<{
            evaluate: SystemEventHandlerRef;
          }> = jasmine.createSpyObj("handler1", ["evaluate"]);
          const spyHandler2: jasmine.SpyObj<{
            evaluate: SystemEventHandlerRef;
          }> = jasmine.createSpyObj("handler2", ["evaluate"]);
          const spyRegistryPrepared = spyOn(
            SystemEventHandlerRegistry,
            "getPreparedArrayOfHandlers",
          ).and.returnValue([
            {
              handlerRef: spyHandler1.evaluate,
              handlerClassInstance: spyHandler1,
            },
            {
              handlerRef: spyHandler2.evaluate,
              handlerClassInstance: spyHandler2,
            },
          ]);
          const spyRegistryFind = spyOn(
            SystemEventHandlerRegistry,
          ...
          expect(spyHandler1.evaluate).toHaveBeenCalledWith(
            payload,
            SE_NAVIGATE,
          );
      ...
      describe("|send|", () => {
        it("should verify will send BLOCKING System Event to 2 Handlers", fakeAsync(() => {
          ...
          // When
          SystemEventDispatcher.send(SE_NAVIGATE, payload).then(() => {
            // No-op.
          });
        ...
        it("should verify will send BLOCKING System Event to 1 Handler and will skip the second one", fakeAsync(() => {
          ...
          // When
          SystemEventDispatcher.send(SE_NAVIGATE, payload, 1).then(() => {
            // No-op.
          });
