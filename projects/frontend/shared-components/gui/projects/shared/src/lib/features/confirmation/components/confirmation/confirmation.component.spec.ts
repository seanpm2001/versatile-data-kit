  ...
  describe("Template::", () => {
    it("should verify will render title, message, confirm button", fakeAsync(() => {
      ...
      const titleElement: HTMLHeadingElement = fixture.debugElement.query(
        By.css(".confirmation__header-title"),
      ).nativeElement;
      const messageDebugElement: DebugElement = fixture.debugElement.query(
        By.css(".confirmation__body-message"),
      );
      const confirmationBtnElement: HTMLButtonElement =
      ...
      // Then
      const messageElement: HTMLParagraphElement = fixture.debugElement.query(
        By.css(".confirmation__body-message"),
      ).nativeElement;
      expect(titleElement.innerHTML).toEqual("<b>Confirmation Test</b>");
      expect(messageElement.innerHTML).toEqual(
        "Test message, that want <b>explicit</b> confirmation!",
      );
      expect(confirmationBtnElement.innerText.trim()).toEqual(
        "Test Confirm Btn Text",
      );
      expect(
        fixture.debugElement.query(By.css(".confirmation__header-close-btn")),
      ).toBeNull();
      expect(
        fixture.debugElement.query(
      ...
      expect(
        fixture.debugElement.query(By.css(".confirmation__footer-cancel-btn")),
      ).toBeNull();
      expect(
        fixture.debugElement.query(
    ...
    it(`should verify won't render message if not supplied`, fakeAsync(() => {
      // Then 1
      let messageDebugElement: DebugElement = fixture.debugElement.query(
        By.css(".confirmation__body-message"),
      );
      expect(messageDebugElement).toBeNull();
      ...
      // Then
      messageDebugElement = fixture.debugElement.query(
        By.css(".confirmation__body-message"),
      );
      expect(messageDebugElement).toBeNull();
    }));
    ...
    it("should verify will render title, close button, message, opt-out checkbox, cancel and confirm buttons with icons", fakeAsync(() => {
      ...
      expect(messageElement.innerHTML).toEqual(
        "Test message, that want <i>explicit</i> confirmation!",
      );
      expect(confirmationBtnElement.innerText.trim()).toEqual(
        "Test Confirm Btn Text Case 1",
      );

      // close button
      const closeDebugElement = fixture.debugElement.query(
        By.css(".confirmation__header-close-btn"),
      );
      const closeIconElement: HTMLElement = closeDebugElement.query(
        By.css("clr-icon"),
      ).nativeElement;
      ...
      // cancel button
      const cancelButtonDebugElement = fixture.debugElement.query(
        By.css(".confirmation__footer-cancel-btn"),
      );
      const cancelButtonElement: HTMLButtonElement =
        cancelButtonDebugElement.nativeElement;
      expect(cancelButtonElement).toBeDefined();
      expect(cancelButtonElement.innerText).toEqual(
        "Test Cancel Btn Text Case 1",
      );
      const buttonCancelIconElement: HTMLElement =
        cancelButtonDebugElement.query(
          By.css(".confirmation__footer-confirm-btn-icon"),
        ).nativeElement;
      ...
      // confirm button
      const confirmButtonDebugElement = fixture.debugElement.query(
        By.css(".confirmation__footer-confirm-btn"),
      );
      const confirmButtonElement: HTMLButtonElement =
        confirmButtonDebugElement.nativeElement;
      expect(confirmButtonElement).toBeDefined();
      expect(confirmButtonElement.innerText).toEqual(
        "Test Confirm Btn Text Case 1",
      );
      const buttonConfirmIconElement: HTMLElement =
        confirmButtonDebugElement.query(
          By.css(".confirmation__footer-confirm-btn-icon"),
        ).nativeElement;
      expect(buttonConfirmIconElement).toBeDefined();
      expect(buttonConfirmIconElement.getAttribute("shape")).toEqual("pop-out");
      expect(buttonConfirmIconElement.getAttribute("direction")).toEqual(
        "down",
      );
      ...
      expect(buttonConfirmIconElement.getAttribute("status")).toEqual(
        "warning",
      );
      ...
      expect(
        cancelButtonElement.nextElementSibling.classList.contains(
          "confirmation__footer-confirm-btn",
        ),
      ).toBeTrue();
    }));
    ...
    const parameters: Array<[string, boolean, string, string, any]> = [
      [
        "close",
        false,
        ".confirmation__header-close-btn",
        "dismiss",
        "Confirmation canceled on User behalf",
      ],
      [
        "cancel",
        false,
        ".confirmation__footer-cancel-btn",
        "dismiss",
        "Confirmation canceled on User behalf",
      ],
      [
    ...
    for (const [
      ...
      it(`should verify ${context} view will invoke ${handlerMethod} handler method and opt-out is ${
        ...
        // Given
        const handlerStub = jasmine.createSpyObj<ConfirmationHandler>(
          "handlerStub",
          ["confirm", "dismiss"],
        );
        const model = new ConfirmationModelImpl({
        ...
        const actionElement: HTMLButtonElement = fixture.debugElement.query(
          By.css(selector),
        ).nativeElement;
        actionElement.click();
        tick(500);
    ...
    it("should verify without user interaction when component is forcefully destroyed will throw Error with code", fakeAsync(() => {
      ...
      // Then
      expect(handlerStub.dismiss).toHaveBeenCalledWith(
