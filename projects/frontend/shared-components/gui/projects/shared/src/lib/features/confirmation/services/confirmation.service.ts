...
export class ConfirmationService extends TaurusObject implements OnDestroy {
  ...
  confirm(model: ConfirmationInputModel): Promise<ConfirmationOutputModel> {
    ...
    const acquireViewContainerRefStatus =
      this._acquireDynamicViewContainerRef();
    if (!acquireViewContainerRefStatus.status) {
      return Promise.reject(new Error(acquireViewContainerRefStatus.error));
    }

    const createConfContainerComponentStatus =
      this._createConfirmationContainerComponent();
    if (!createConfContainerComponentStatus.status) {
      return Promise.reject(
        new Error(createConfContainerComponentStatus.error),
      );
    }

    const createConfComponentStatus =
      this._createConfirmationComponent(modelImpl);
    ...
    return promise
      ...
      .finally(() => {
        this._clearSingleConfirmationResources(
          createConfComponentStatus as {
  ...
  private _acquireDynamicViewContainerRef(): {
    ...
    if (!this._acquiredViewContainerRef) {
      const acquiredViewContainerRef =
    ...
    if (!this._confirmationContainerRef) {
      try {
        this._confirmationContainerRef =
          this._acquiredViewContainerRef.viewContainerRef.createComponent(
            ConfirmationContainerComponent,
          );
        this._confirmationContainerRef.changeDetectorRef.detectChanges();
      } catch (e) {
  ...
  private _createConfirmationComponent(model: ConfirmationModelImpl): {
    ...
    try {
      const confirmationComponentRef =
        this._confirmationContainerRef.instance.viewContainerRef.createComponent(
          ConfirmationComponent,
        );

      const assignMessageAndModelStatus =
        ConfirmationService._assignMessageAndModel(
  ...
  private _isAcquiredViewContainerRefUnhealthy(): boolean {
    return (
      this._acquiredViewContainerRef &&
      this._acquiredViewContainerRef.hostView &&
      this._acquiredViewContainerRef.hostView.destroyed
    );
  }

  private _refineConfirmationContainerVisibility(forceHide = false): void {
    if (
      this._confirmationContainerRef.instance.viewContainerRef.length === 0 ||
      forceHide
    ) {
  ...
  private _clearSingleConfirmationResources(internalModelRef: {
    componentRef: ComponentRef<ConfirmationComponent>;
  }): void {
    try {
      const foundViewRefIndex =
        this._confirmationContainerRef.instance.viewContainerRef.indexOf(
          internalModelRef.componentRef.hostView,
        );
      if (foundViewRefIndex !== -1) {
        this._confirmationContainerRef.instance.viewContainerRef.remove(
          foundViewRefIndex,
        );
      }
  ...
  private _clearResources(): void {
    try {
      if (
        this._confirmationContainerRef?.hostView &&
        !this._confirmationContainerRef.hostView.destroyed
      ) {
    ...
    this.dynamicComponentsService.destroyUniqueViewContainerRef(
      this._acquiredViewContainerRef.uuid,
    );
  ...
  private static _assignMessageAndModel(
    ...
    if (CollectionsUtil.isDefined(model.messageComponent)) {
      try {
        const messageComponentRef =
          confirmationComponentRef.instance.viewContainerRef.createComponent(
