  ...
  implements OnDestroy
{
   ...
   * @private
   */
  private readonly _uniqueComponentRefsStore: Map<
    string,
  ...
  getUniqueViewContainerRef(
  ...
  ): AcquireViewContainerRefModel {
    const isContainerComponentSuccessfullyCreated =
      this._createDynamicContainerComponent();
    if (!isContainerComponentSuccessfullyCreated) {
      return null;
    }

    const isContainerComponentHealthy =
      this._validateDynamicContainerComponent();
    ...
    if (!this._uniqueComponentRefsStore.has(uuid)) {
      const isContextComponentSuccessfullyCreated =
        this._createDynamicContextComponent(uuid);
      if (!isContextComponentSuccessfullyCreated) {
        return null;
      }
    }

    const isContextComponentHealthy =
      this._validateDynamicContextComponent(uuid);
    ...
    return {
      uuid: uuid,
      viewContainerRef:
        this._uniqueComponentRefsStore.get(uuid).instance.viewContainerRef,
  ...
  private _createDynamicContainerComponent(): boolean {
    ...
    if (
      this._dynamicContainerRef &&
      this._dynamicContainerRef.hostView &&
      !this._dynamicContainerRef.hostView.destroyed
    ) {
      return true;
    }

    try {
      this._dynamicContainerRef = this._appRootViewContainerRef.createComponent(
        DynamicContainerComponent,
      );
  ...
  private _validateDynamicContainerComponent(): boolean {
    if (
      !(
        this._dynamicContainerRef &&
        this._dynamicContainerRef.instance &&
        this._dynamicContainerRef.instance.viewContainerRef
      )
    ) {
      console.error(
  ...
  private _createDynamicContextComponent(uuid: string): boolean {
    try {
      const uniqueDynamicComponentRef =
        this._dynamicContainerRef.instance.viewContainerRef.createComponent(
          DynamicContextComponent,
        );
  ...
  private _validateDynamicContextComponent(uuid: string): boolean {
    ...
    if (
      !(
        ...
        retrievedComponentRef.instance &&
        retrievedComponentRef.instance.viewContainerRef
      )
    ) {
      console.error(
