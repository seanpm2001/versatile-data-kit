  ...
  implements OnInit, OnChanges
{
   ...
   */
  @ViewChild("errorTemplateSystemDefault", { read: TemplateRef })
  errorTemplateRefSystemDefault: TemplateRef<never>;
   ...
   */
  @ContentChild("errorTemplate", { read: TemplateRef })
  errorTemplateRefGeneric: TemplateRef<never>;
   ...
   *          - fallback to system default Error Template.
   */
  @ContentChild("errorTemplate4xx", { read: TemplateRef })
  errorTemplateRefClientErrors: TemplateRef<never>;
   ...
   *          - fallback to system default Error Template.
   */
  @ContentChild("errorTemplate400", { read: TemplateRef })
  errorTemplateRefBadRequest: TemplateRef<never>;
   ...
   *          - fallback to system default Error Template.
   */
  @ContentChild("errorTemplate401", { read: TemplateRef })
  errorTemplateRefUnauthorized: TemplateRef<never>;
   ...
   *          - fallback to system default Error Template.
   */
  @ContentChild("errorTemplate403", { read: TemplateRef })
  errorTemplateRefForbidden: TemplateRef<never>;
   ...
   *          - fallback to system default Error Template.
   */
  @ContentChild("errorTemplate404", { read: TemplateRef })
  errorTemplateRefNotFound: TemplateRef<never>;
   ...
   *          - fallback to system default Error Template.
   */
  @ContentChild("errorTemplate405", { read: TemplateRef })
  errorTemplateRefMethodNotAllowed: TemplateRef<never>;
   ...
   *          - fallback to system default Error Template.
   */
  @ContentChild("errorTemplate409", { read: TemplateRef })
  errorTemplateRefConflict: TemplateRef<never>;
   ...
   *          - fallback to system default Error Template.
   */
  @ContentChild("errorTemplate422", { read: TemplateRef })
  errorTemplateRefUnprocessableEntity: TemplateRef<never>;
   ...
   *          - fallback to system default Error Template.
   */
  @ContentChild("errorTemplate5xx", { read: TemplateRef })
  errorTemplateRefServerErrors: TemplateRef<never>;
   ...
   *          - fallback to system default Error Template.
   */
  @ContentChild("errorTemplate500", { read: TemplateRef })
  errorTemplateRefInternalServerError: TemplateRef<never>;
   ...
   *          - fallback to system default Error Template.
   */
  @ContentChild("errorTemplate503", { read: TemplateRef })
  errorTemplateRefServiceUnavailable: TemplateRef<never>;
  ...
  emptyTemplateRef: TemplateRef<never>;
  ...
  constructor(
    ...
    @Inject(SHARED_FEATURES_CONFIG_TOKEN)
    private readonly featureConfig: PlaceholderConfig,
  ) {
    super();

    const serviceRequestUrl = featureConfig?.placeholder?.serviceRequestUrl
      ? featureConfig.placeholder.serviceRequestUrl
      : "#";

    ErrorEscalation.Generic = CollectionsUtil.interpolateString(
      ErrorEscalation.Generic,
      {
    ...
    ErrorEscalation.NotFound = CollectionsUtil.interpolateString(
      ErrorEscalation.NotFound,
      {
        searchValue: "%service_req_url%",
        replaceValue: serviceRequestUrl,
      },
  ...
  resolveErrorTemplate(
    identifiedErrorRecordWithMessage: IdentifiedErrorRecordWithMessage,
  ): TemplateRef<never> {
    const httpStatusCode =
      identifiedErrorRecordWithMessage?.record[0]?.httpStatusCode;
    ...
    if (
      httpStatusCode >= 400 &&
      httpStatusCode < 500 &&
      this.errorTemplateRefClientErrors instanceof TemplateRef
    ) {
    ...
    if (
      httpStatusCode >= 500 &&
      this.errorTemplateRefServerErrors instanceof TemplateRef
    ) {
  ...
  private _executeRefineCycle(): void {
    try {
      ...
      this.placeholderService.refineElementsState(
        this.elementRef,
        this._hideDefaultEmptyStateImageInGrid,
      );
    } catch (e) {
      console.error(e);
    }
  ...
  private _refineErrorState(): void {
    ...
    const filteredErrorRecords = filterErrorRecords(
      this.errorsQueue,
    ...
    this._hideDefaultEmptyStateImageInGrid =
      this.showError || this.hideDefaultEmptyStateImageInGrid;
  ...
  private _populateTransportBuffers(errorRecords: ErrorRecord[]): void {
    ...
    this.identifiedErrorWithApiMessage =
      this._getErrorsToIdentifiedError(errorRecords);
    this.identifiedErrorsWithApiMessage = errorRecords.map((record) =>
      this._getErrorToIdentifiedError(record),
    );
  }

  private _getErrorToIdentifiedError(
    record: ErrorRecord,
  ): IdentifiedErrorRecordWithMessage {
    if (window && window.navigator && !window.navigator.onLine) {
      return {
        ...
        impactedServices: "",
        apiMessage: this.placeholderService.extractErrorInformation(
          record.error,
        ),
    ...
    if (record && record.httpStatusCode === HttpStatusCode.NotFound) {
      return {
        record: [record],
        problem: CollectionsUtil.interpolateString(
          ErrorProblem.NotFound,
          this._errorContext,
          this.plural ? "are" : "is",
        ),
  ...
  private _getErrorsToIdentifiedError(
    ...
    filteredErrorsTmp = errorRecords.filter(
      (r) => r.httpStatusCode === HttpStatusCode.ServiceUnavailable,
    );
    if (filteredErrorsTmp.length > 0) {
      ...
      if (
        latestIdentifiedError.code === filteredErrorsTmp[0].code ||
        latestIdentifiedError.time - filteredErrorsTmp[0].time < 2000
      ) {
    ...
    filteredErrorsTmp = errorRecords.filter(
      (r) => r.httpStatusCode === HttpStatusCode.InternalServerError,
    );
    ...
    filteredErrorsTmp = errorRecords.filter(
      (r) => r.httpStatusCode === HttpStatusCode.NotFound,
    );
    if (filteredErrors.length === 0 && filteredErrorsTmp.length > 0) {
    ...
    return {
      ...
      escalation: ErrorEscalation.Generic,
      impactedServices:
        PlaceholderService.extractClassesPublicNames(filteredErrors),
      apiMessage: this.placeholderService.extractErrorInformation(
        filteredErrors[0].error,
      ),
      imageSrc: ErrorImgSource.Generic,
  ...
  private static _isPropertyChanged(
