...
class PlaceholderComponentStub implements OnInit {
  @HostListener("changeHideDefaultStateImageInGrid", ["$event"])
  changeHideImageInGrid($event: { value: boolean }) {
...
describe("PlaceholderService", () => {
  ...
  beforeEach(() => {
    ...
    gridDebugElement = fixture.debugElement.query(
      By.directive(ClarityDataGridComponentStub),
    );
    gridComponent = gridDebugElement.componentInstance;

    placeholderDebugElement = fixture.debugElement.query(
      By.directive(PlaceholderComponentStub),
    );
  ...
  describe("Statics::", () => {
    describe("Methods::", () => {
      ...
      beforeEach(() => {
        errorCodes = [
          generateErrorCode(
            PlaceholderService.CLASS_NAME,
            PlaceholderService.PUBLIC_NAME,
            "refineElementsState",
            "500",
          ),
          generateErrorCode(
      ...
      describe("|extractClassPublicName|", () => {
        it("should verify will extract Class Public Name", () => {
          // When
          const publicName1 = PlaceholderService.extractClassPublicName(
            errorRecords[0],
          );
          const publicName2 = PlaceholderService.extractClassPublicName(
            errorRecords[1],
          );
          const publicName3 = PlaceholderService.extractClassPublicName(
            errorRecords[2],
          );
      ...
      describe("|extractClassesPublicNames|", () => {
        it("should verify will extract Class Public Names of multiple ErrorRecord", () => {
          // When
          const publicNames =
            PlaceholderService.extractClassesPublicNames(errorRecords);

          // Then
          expect(publicNames).toEqual(
            "Placeholder service, Data load api service",
          );
        });
      });
    });
    ...
    describe("|refineElementsState|", () => {
      it("should verify method would be invoked from stub Component upon change detection", () => {
        ...
        expect(spyMethod).toHaveBeenCalledWith(
          placeholderComponent.elementRef,
          false,
        );
      });
      ...
      it("should verify will append styling in head", fakeAsync(() => {
        ...
        // Then
        const styleElementGrid = document.querySelector(
          `head style[data-shared-grid-style="${randomStringGrid}"]`,
        );
      ...
      it("should verify wont append styling in head if grid is not found", fakeAsync(() => {
        // Given
        const spyRenderer2ParentNode = spyOn(
          renderer2,
          "parentNode",
      ...
      it("should verify will append styling in head if element is standalone", fakeAsync(() => {
        ...
        expect(
          styleElementStandalone.innerHTML.trim().replace(/\s/g, ""),
        ).toEqual(
          `
      ...
      it("should verify will toggle appended styling in head depending of provided flag to method", fakeAsync(() => {
        ...
        // When 2
        placeholderDebugElement.triggerEventHandler(
          "changeHideDefaultStateImageInGrid",
          { value: true },
        );
      ...
      it("should verify will append data attribute to grid component when found", fakeAsync(() => {
        ...
        // Then
        expect(
          gridComponent.elementRef.nativeElement.hasAttribute(
            "data-shared-grid",
          ),
        ).toBeTrue();
        expect(
          gridComponent.elementRef.nativeElement.getAttribute(
            "data-shared-grid",
          ),
        ).toEqual(randomStringGrid);
      }));
      ...
      it("should verify will append data attribute to placeholder when is standalone", fakeAsync(() => {
        ...
        // Then
        expect(
          placeholderComponent.elementRef.nativeElement.hasAttribute(
            "data-shared-placeholder",
          ),
        ).toBeTrue();
        expect(
          placeholderComponent.elementRef.nativeElement.getAttribute(
            "data-shared-placeholder",
          ),
        ).toEqual(randomStringStandalone);
      }));
    });
