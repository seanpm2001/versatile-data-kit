...
export class PlaceholderService extends TaurusObject implements OnDestroy {
  ...
  constructor(private readonly renderer2: Renderer2) {
    ...
    this._standalonePlaceholderRandomAttribute =
      CollectionsUtil.generateRandomString();
  }
  ...
  static extractClassesPublicNames(errorRecords: ErrorRecord[]): string {
    const publicNames = errorRecords
      ...
      .filter(
        (publicName) =>
          CollectionsUtil.isString(publicName) && publicName.length > 0,
      );
      ...
      if (errorRecord.code && errorRecord.code.length > 0) {
        ...
        if (codeChunks.length >= 3) {
          ...
          if (publicName && publicName.length > 2) {
            ...
            return (
              publicNameNormalized.substring(0, 1).toUpperCase() +
              publicNameNormalized.substring(1).toLowerCase()
            );
          }
        }
      }
  ...
  refineElementsState(
    ...
    hideDefaultEmptyStateImageInGrid: boolean,
  ): void {
  ...
  private _findPlaceholderParentGrid(): Promise<HTMLElement> {
    ...
    const parentFinder = () => {
      ...
      const firstLevelParent: HTMLElement = this.renderer2.parentNode(
        this._elementRef.nativeElement,
      ) as HTMLElement;
      if (firstLevelParent) {
        const foundParentGrid =
          this._traverseToFindParentGrid(firstLevelParent);
  ...
  private _appendGridPlaceholderStyle(): void {
    if (!this._gridStyleElement) {
      this._gridStyleElement = this.renderer2.createElement(
        "style",
      ) as HTMLStyleElement;

      this.renderer2.setAttribute(
        this._gridStyleElement,
        "data-shared-grid-style",
        this._gridRandomAttribute,
      );
  ...
  private _appendStandalonePlaceholderStyle(): void {
    if (!this._standalonePlaceholderStyleElement) {
      this._standalonePlaceholderStyleElement = this.renderer2.createElement(
        "style",
      ) as HTMLStyleElement;
  ...
  private _toggleGridPlaceholderStyle(): void {
    ...
    this._gridStyleElement.innerHTML = `
            clr-datagrid[data-shared-grid="${
              this._gridRandomAttribute
            }"] clr-dg-placeholder .datagrid-placeholder.datagrid-empty {
                justify-content: center;
            }
            clr-datagrid[data-shared-grid="${
              this._gridRandomAttribute
            }"] clr-dg-placeholder .datagrid-placeholder-image {
                display: ${
                  this._hideDefaultEmptyStateImageInGrid ? "none" : "block"
                };
            }
        `;
  }
