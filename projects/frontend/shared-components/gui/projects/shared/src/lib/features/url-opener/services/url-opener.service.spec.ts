...
describe("UrlOpenerService", () => {
  ...
  beforeEach(() => {
    ...
    confirmationServiceStub = jasmine.createSpyObj<ConfirmationService>(
      "confirmationServiceStub",
      ["confirm"],
    );
    cookieServiceStub = jasmine.createSpyObj<CookieService>(
      "cookieServiceStub",
      ["get", "set"],
    );
  ...
  describe("Methods::", () => {
    describe("|initialize|", () => {
      const parameters = [
        ...
        [
          btoa('{"name":"auserov'),
          `${UrlOpenerService.CLASS_NAME}: Potential bug found, cannot parse provided JSON`,
        ],
    ...
    describe("|open|", () => {
      describe("Internal_Url::", () => {
        ...
        it("should verify will open url and target", (done) => {
          // Given
          const url =
            "http://localhost:9876/pathname/context/12?param1=value10&param2=value20";
          navigationServiceStub.navigate.and.returnValue(Promise.resolve(true));

          // When
          service.open(url, "_self").then((value) => {
            expect(value).toBeTrue();
            expect(navigationServiceStub.navigate).toHaveBeenCalledWith(
              "http://localhost:9876/pathname/context/12",
              {
                queryParams: {
                  param1: "value10",
                  param2: "value20",
        ...
        it(`should verify will open url and target _self and won't throw error when query extraction fails`, (done) => {
          // Given
          const url =
            "http://localhost:9876/pathname/context/14?param1=value10&param2=value20";
          const consoleErrorSpy = spyOn(console, "error").and.callFake(
            CallFake,
          );
          spyOn(URLSearchParams.prototype, "forEach").and.throwError(
            new Error("error"),
          );
          navigationServiceStub.navigate.and.returnValue(Promise.resolve(true));
          ...
          service.open(url, "_self").then((value) => {
            ...
            expect(navigationServiceStub.navigate).toHaveBeenCalledWith(
              "http://localhost:9876/pathname/context/14",
              {
                queryParams: {},
              },
            );
        ...
        it(`should verify will open url and target _blank and will leverage persisted data`, (done) => {
          ...
          cookieServiceStub.get.and.returnValue(
            btoa(
              JSON.stringify({
                "http://localhost:9876/pathname/context/16": "1",
              }),
            ),
          );
          service.initialize();
        ...
        it("should verify will open url, target _blank and confirmation model", (done) => {
          ...
          confirmationServiceStub.confirm.and.returnValue(
            Promise.resolve({ doNotShowFutureConfirmation: true }),
          );
          ...
          service
            ...
            .then((value) => {
              ...
              expect(cookieServiceStub.set).toHaveBeenCalledWith(
                ...
                btoa(
                  JSON.stringify({
                    "http://localhost:9876/pathname/context/18": "1",
                  }),
                ),
              );
              done();
        ...
        it(`should verify will reject if cannot open external url`, (done) => {
          ...
          confirmationServiceStub.confirm.and.returnValue(
            Promise.resolve({ doNotShowFutureConfirmation: false }),
          );
          ...
          service
            ...
            .catch((reason) => {
              ...
              expect(reason).toEqual(
                new Error(
                  `${UrlOpenerService.CLASS_NAME}: Exception thrown, cannot open Super Collider url in a new tab`,
                ),
              );
              done();
            });
      ...
      describe("External_Url::", () => {
        ...
        it(`should verify will open confirmation and after will open url, and will persist opt-out data for non-english origin`, (done) => {
          ...
          service.open(url).then((value) => {
            ...
            expect(cookieServiceStub.set).toHaveBeenCalledWith(
              ...
              btoa(
                JSON.stringify({
                  "http://unknown.something.xn--80aei0bjen": "1",
                }),
              ),
            );
            done();
        ...
        it(`should verify will open confirmation and after will open url, target and model, and won't persist opt-out data because cannot be encoded base64`, (done) => {
          ...
          spyOnProperty(URL.prototype, "origin", "get").and.returnValue(
            "аусеров",
          );
        ...
        it(`should verify will reject if cannot open external url`, (done) => {
          ...
          service
            ...
            .catch((reason) => {
              ...
              expect(reason).toEqual(
                new Error(
                  `${UrlOpenerService.CLASS_NAME}: Exception thrown cannot open external url`,
                ),
              );
              done();
            });
