...
export class UrlOpenerService extends TaurusObject {
  ...
  constructor(
    ...
    try {
      ...
      this._cookieKey = UrlOpenerService._encodeBase64(
        UrlOpenerService.CLASS_NAME,
      );
    } catch (e) {
      console.error(
        `${UrlOpenerService.CLASS_NAME}: Potential bug found, cannot identify Cookie key and Origin`,
      );
    }
  }
  ...
  open(
    url: string,
    target: UrlOpenerTarget = "_blank",
    model: UrlOpenerModel = null,
  ): Promise<boolean> {
  ...
  private _executeExternalNavigation(
    ...
    if (urlOriginData === "1") {
      ...
      if (CollectionsUtil.isNil(model.confirmBtnModel)) {
        ...
        !CollectionsUtil.isStringWithContent(model.confirmBtnModel.text)
      ) {
    ...
    return confirmationPromise.then((data) => {
      ...
      const isSuccessful = UrlOpenerService._openExternalUrl(
        nextStep.sanitizedUrl,
        nextStep.newTab,
      );
      if (isSuccessful) {
        return true;
  ...
  private _executeInternalNavigation(
    ...
    return confirmationPromise.then((data) => {
      ...
      return this.navigationService.navigate(_sanitizedUrl, {
  ...
  private _extractSkippedUrlsMap(): Record<string, "1"> {
    ...
    const decodedOriginsMap =
      UrlOpenerService._decodeBase64(extractedOriginsMap);
    if (!decodedOriginsMap) {
      return null;
    }

    const parsedOriginsMap =
      UrlOpenerService._parseToJSON<Record<string, "1">>(decodedOriginsMap);
    if (!parsedOriginsMap) {
      return null;
    }
  ...
  private _persistSkippedUrlsMap(): void {
    ...
    const serializedOriginsMap = UrlOpenerService._serializeObject(
      this._skippedOriginsUrlsMap,
    );
    if (!serializedOriginsMap) {
      return;
    }

    const encodedOriginsMap =
      UrlOpenerService._encodeBase64(serializedOriginsMap);
