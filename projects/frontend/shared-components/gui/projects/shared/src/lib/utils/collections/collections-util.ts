...
export class CollectionsUtil {
  ...
  static isDefined<T>(value: T): boolean {
    return (
      !CollectionsUtil.isNull(value) && !CollectionsUtil.isUndefined(value)
    );
  }
   ...
   */
  static isPrimitiveOrDate<T extends PrimitivesDate>(
    value: T | any,
  ): value is Primitives | Nil | Date {
  ...
  static isFunction<T extends (...args: any[]) => any>(
    value: T | any,
   ...
   */
  static isObject<T extends Record<string, unknown>>(
    obj: T | any,
   ...
   */
  static isLiteralObjectOrNull<T extends Record<string, unknown>>(
    obj: T | any,
  ): obj is LiteralObjectOrNull {
    return (
   ...
   */
  static isLiteralObject<T extends Record<string, unknown>>(
    obj: T | any,
  ): boolean {
    return (
      CollectionsUtil.isLiteralObjectOrNull(obj) && !CollectionsUtil.isNull(obj)
    );
  }
   ...
   */
  static isObjectWithProperties<T extends Record<string, unknown>>(
    obj: T | any,
  ): boolean {
   ...
   */
  static isLiteralObjectWithProperties<T extends Record<string, unknown>>(
    obj: T | unknown,
  ): boolean {
    return (
      CollectionsUtil.isLiteralObject<T>(obj) && Object.keys(obj).length > 0
    );
  }
   ...
   */
  static objectValues<T extends Record<string, any>>(
   ...
   */
  static transformMapToObject<T extends Map<string, unknown>>(
    map: T,
  ): { [key: string]: unknown } {
   ...
   */
  static transformObjectToMap<T extends { [key: string]: any }>(
    obj: T,
  ): Map<string, any> {
   ...
   */
  static objectPairs<T extends Record<keyof T, T[keyof T]>>(
    obj: T | null | undefined,
   ...
   */
  static getObjectPropertyDescriptor<T extends Record<string, any>>(
    obj: T,
    key: string,
  ): PropertyDescriptor {
    if (
      !(
        (CollectionsUtil.isFunction(obj) || CollectionsUtil.isObject(obj)) &&
        CollectionsUtil.isString(key)
      )
    ) {
      return null;
    }
  ...
  static iterateClassStatics<T extends Record<string, any>>(
    ...
    if (
      !CollectionsUtil.isFunction(fn) ||
      !CollectionsUtil.isFunction(iteratorFn)
    ) {
      return null;
    }
  ...
  static areMapsEqual(
    ...
    const evaluateDeepComparison = () => {
      for (const [key, val] of m1) {
        ...
        if (CollectionsUtil.isMap(val)) {
          if (
            !CollectionsUtil.areMapsEqual(
              val,
              compareVal as Map<unknown, unknown>,
            )
          ) {
            return false;
          }
        ...
        if (
          !CollectionsUtil.isEqual(val, compareVal) ||
          (CollectionsUtil.isUndefined(compareVal) && !m2.has(key))
        ) {
          return false;
        }
      }
    ...
    return (
      CollectionsUtil.isMap(m1) &&
      CollectionsUtil.isMap(m2) &&
      (m1 === m2 || (m1.size === m2.size && evaluateDeepComparison()))
    );
  }
  ...
  static interpolateString(
    target: string,
    ...replacers: string[] | Array<Replacer<string>>
  ): string {
    let response = target;
