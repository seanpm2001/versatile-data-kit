 ...
 */
export type ArrayIterator<TCollection, TResult> = (
  value: TCollection,
  index: number,
 ...
 */
export type ArrayMapper<TCollection, TResult> = (
  value: TCollection,
  index: number,
  collection: TCollection[],
) => TResult;
 ...
 */
export type ObjectPredicateFn<TObject> = (
  value: TObject[keyof TObject],
  key?: string,
  collection?: TObject,
) => boolean;
 ...
 */
export type ArrayPredicateFn<TArray extends any[]> = (
...
export type FilterValues<TObject, TCondition, KExclude extends keyof any> = {
  [key in keyof Omit<TObject, KExclude>]: Omit<
    TObject,
    KExclude
  >[key] extends TCondition
    ? key
    : never;
};
 ...
 *      - exclude (omit) specific keys from object
 */
export type FilterValuesNegation<
  TObject,
  TCondition,
  KExclude extends keyof any,
> = {
  [key in keyof Omit<TObject, KExclude>]: Omit<
    TObject,
    KExclude
 ...
 */
export type FilterMethodNames<
  TClass,
  KExclude extends keyof any,
> = FilterValues<TClass, Function, KExclude>[keyof FilterValues<
  TClass,
  Function,
  KExclude
>];
 ...
 */
export type FilterPropertyNames<
  TClass,
  KExclude extends keyof any,
> = FilterValuesNegation<TClass, Function, KExclude>[keyof FilterValuesNegation<
  TClass,
  Function,
  KExclude
>];
